...
PYBIND11_MODULE(tinyobjloader, tobj_module) {
  ...
  py::class_<ObjReader>(tobj_module, "ObjReader")
      .def(py::init<>())
      .def("ParseFromFile", &ObjReader::ParseFromFile, py::arg("filename"),
           py::arg("option") = ObjReaderConfig())
      .def("ParseFromString", &ObjReader::ParseFromString, py::arg("obj_text"),
           py::arg("mtl_text"), py::arg("option") = ObjReaderConfig())
  ...
  py::class_<attrib_t>(tobj_module, "attrib_t")
      ...
      .def("numpy_vertices",
           [](attrib_t &instance) {
             ...
             memcpy(buf.ptr, instance.vertices.data(),
                    instance.vertices.size() * sizeof(real_t));
             return ret;
           })
  ...
  // NOTE(syoyo): It looks it is rather difficult to expose assignment by array
  // index to python world for array variable. For example following python
  // scripting does not work well.
  //
  // print(mat.diffuse)
  // >>> [0.1, 0.2, 0.3]
  // mat.diffuse[1] = 1.0
  ...
  py::class_<material_t>(tobj_module, "material_t")
      ...
      .def_property("specular", &material_t::GetSpecular,
                    &material_t::SetSpecular)
      .def_property("transmittance", &material_t::GetTransmittance,
                    &material_t::SetTransmittance)
      ...
      .def_readwrite("specular_highlight_texname",
                     &material_t::specular_highlight_texname)
  ...
  py::class_<mesh_t>(tobj_module, "mesh_t")
      ...
      .def("numpy_num_face_vertices",
           [](mesh_t &instance) {
             auto ret =
                 py::array_t<unsigned char>(instance.num_face_vertices.size());
             py::buffer_info buf = ret.request();
             memcpy(buf.ptr, instance.num_face_vertices.data(),
                    instance.num_face_vertices.size() * sizeof(unsigned char));
             return ret;
           })
      .def_readonly("indices", &mesh_t::indices)
      .def("numpy_indices",
           [](mesh_t &instance) {
             // Flatten indexes. index_t is composed of 3 ints(vertex_index,
             // normal_index, texcoord_index). numpy_indices = [0, -1, -1, 1,
             // -1, -1, ...] C++11 or later should pack POD struct tightly and
             // does not reorder variables, so we can memcpy to copy data.
             // Still, we check the size of struct and byte offsets of each
             // variable just for sure.
             static_assert(sizeof(index_t) == 12, "sizeof(index_t) must be 12");
             static_assert(offsetof(index_t, vertex_index) == 0,
                           "offsetof(index_t, vertex_index) must be 0");
             static_assert(offsetof(index_t, normal_index) == 4,
                           "offsetof(index_t, normal_index) must be 4");
             static_assert(offsetof(index_t, texcoord_index) == 8,
                           "offsetof(index_t, texcoord_index) must be 8");

             auto ret = py::array_t<int>(instance.indices.size() * 3);
             py::buffer_info buf = ret.request();
             memcpy(buf.ptr, instance.indices.data(),
                    instance.indices.size() * 3 * sizeof(int));
             return ret;
           })
      ...
      .def("numpy_material_ids", [](mesh_t &instance) {
        ...
        py::buffer_info buf = ret.request();
        memcpy(buf.ptr, instance.material_ids.data(),
               instance.material_ids.size() * sizeof(int));
        return ret;
      });

  py::class_<lines_t>(tobj_module, "lines_t").def(py::init<>());

  py::class_<points_t>(tobj_module, "points_t").def(py::init<>());
}
