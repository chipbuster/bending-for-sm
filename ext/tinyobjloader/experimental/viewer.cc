  ...
  if (strcmp(ext, ".gz") == 0) {
    ...
    if (ret) {
      char* p = static_cast<char*>(
          malloc(buf.size() + 1));  // @fixme { implement deleter }
...
bool LoadObjAndConvert(float bmin[3], float bmax[3], const char* filename,
...

  {
    ...
    for (size_t v = 0; v < attrib.face_num_verts.size(); v++) {
      assert(attrib.face_num_verts[v] % 3 ==
             0);  // assume all triangle face(multiple of 3).
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cout << "view input.obj <num_threads> <benchark_only> <verbose>"
              << std::endl;
    return 0;
  }
