...
void print_header(std::string title) {
  ...
  std::cout << std::string((width - title.length()) / 2, ' ') << title
            << std::endl;
...
void print_row(int n, int dataset, double time_f77, double err_f77,
               int nops_f77, double time_cpp, double err_cpp, int nops_cpp) {
  const char sep = ' ';
...
void run_eigs_sym(int n_experiment, int n_replicate, int n, int k, int m) {
  ...
  for (int i = 0; i < n_experiment; i++) {
    ...
    for (int j = 0; j < n_replicate; j++) {
      ...
      print_row(n, i + 1, time_f77, err_f77, nops_f77, time_cpp, err_cpp,
                nops_cpp);
    }
  }
}
