...
void eigs_sym_F77(MatrixXd &M, VectorXd &init_resid, int k, int m,
  ...
  saupd(ido, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam, ipntr, workd,
        workl, lworkl, info);
  ...
  while (ido == -1 || ido == 1) {
    ...
    saupd(ido, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam, ipntr,
  ...
  seupd(rvec, howmny, d, Z, ldz, sigma, bmat, n, which, nev, tol, resid, ncv, V,
  ...
  /*
     ...
     std::endl; std::cout << "first 5 rows of computed eigenvectors U = \n" <<
  ...
  */
  ...
  time_used = (end - start) * 1000;
  MatrixXd err =
      M * evecs.leftCols(nev) - evecs.leftCols(nev) * evals.asDiagonal();
...
void eigs_gen_F77(MatrixXd &M, VectorXd &init_resid, int k, int m,
  ...
  naupd(ido, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam, ipntr, workd,
        workl, lworkl, info);
  // ido == -1 or ido == 1 means more iterations needed
  while (ido == -1 || ido == 1) {
    MapVec vec_in(&workd[ipntr[0] - 1], n);
    ...
    naupd(ido, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam, ipntr,
          workd, workl, lworkl, info);
  }
  ...
  // Use seupd() to retrieve results
  neupd(rvec, howmny, dr, di, Z, ldz, sigmar, sigmai, workv, bmat, n, which,
