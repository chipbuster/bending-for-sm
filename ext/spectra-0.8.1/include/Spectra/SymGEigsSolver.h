...
///   Cholesky solver.)
///   See \ref SymGEigsSolver<Scalar, SelectionRule, OpType, BOpType,
///   GEIGS_CHOLESKY> "SymGEigsSolver (Cholesky mode)" for more details.
/// - The regular inverse mode requires the matrix-vector product \f$Bv\f$ and
/// the
...
///   when computing \f$B^{-1}v\f$ is much faster than the Cholesky
///   decomposition. See \ref SymGEigsSolver<Scalar, SelectionRule, OpType,
///   BOpType, GEIGS_REGULAR_INVERSE> "SymGEigsSolver (Regular inverse mode)"
///   for more details.
...
/// where \f$A\f$ is symmetric and \f$B\f$ is positive definite with the
/// Cholesky decomposition \f$B=LL'\f$.
///
/// This solver requires two matrix operation objects: one for \f$A\f$ that
/// implements the matrix multiplication \f$Av\f$, and one for \f$B\f$ that
/// implements the lower and upper triangular solving \f$L^{-1}v\f$ and
/// \f$(L')^{-1}v\f$.
///
/// If \f$A\f$ and \f$B\f$ are stored as Eigen matrices, then the first
/// operation can be created using the DenseSymMatProd or SparseSymMatProd
/// classes, and the second operation can be created using the DenseCholesky or
/// SparseCholesky classes. If the users need to define their own operation
/// classes, then they should implement all the public member functions as in
/// those built-in classes.
///
...
///                       \ref Enumerations.
/// \tparam OpType        The name of the matrix operation class for \f$A\f$.
/// Users could either
...
///                       in DenseSymMatProd.
/// \tparam BOpType       The name of the matrix operation class for \f$B\f$.
/// Users could either
...
/// int main()
/// {
///     // We are going to solve the generalized eigenvalue problem A * x =
///     lambda * B * x const int n = 100;
///
///     // Define the A matrix
///     Eigen::MatrixXd M = Eigen::MatrixXd::Random(n, n);
///     Eigen::MatrixXd A = M + M.transpose();
///
///     // Define the B matrix, a band matrix with 2 on the diagonal and 1 on
///     the subdiagonals Eigen::SparseMatrix<double> B(n, n);
...
///
///     // Construct generalized eigen solver object, requesting the largest
///     three generalized eigenvalues SymGEigsSolver<double, LARGEST_ALGE,
///     DenseSymMatProd<double>, SparseCholesky<double>, GEIGS_CHOLESKY>
///         geigs(&op, &Bop, 3, 6);
///
...
///
///     std::cout << "Generalized eigenvalues:\n" << es.eigenvalues().tail(3) <<
///     std::endl; std::cout << "Generalized eigenvectors:\n" <<
...
class SymGEigsSolver<Scalar, SelectionRule, OpType, BOpType, GEIGS_CHOLESKY>
    : public SymEigsBase<Scalar, SelectionRule,
                         SymGEigsCholeskyOp<Scalar, OpType, BOpType>,
                         IdentityBOp> {
private:
...
public:
  ...
  ///             should implement the matrix-vector multiplication operation of
  ///             \f$A\f$: calculating \f$Av\f$ for any vector \f$v\f$. Users
  ///             could either create the object from the wrapper classes such
  ///             as DenseSymMatProd, or define their own that implements all
  ///             the public member functions as in DenseSymMatProd.
  ...
  ///             create the object from the wrapper classes such as
  ///             DenseCholesky, or define their own that implements all the
  ...
  SymGEigsSolver(OpType *op, BOpType *Bop, Index nev, Index ncv)
      : SymEigsBase<Scalar, SelectionRule,
                    SymGEigsCholeskyOp<Scalar, OpType, BOpType>, IdentityBOp>(
            new SymGEigsCholeskyOp<Scalar, OpType, BOpType>(*op, *Bop), NULL,
            nev, ncv),
        m_Bop(Bop) {}
  ...
  Matrix eigenvectors(Index nvec) const {
    Matrix res = SymEigsBase<Scalar, SelectionRule,
                             SymGEigsCholeskyOp<Scalar, OpType, BOpType>,
                             IdentityBOp>::eigenvectors(nvec);
    Vector tmp(res.rows());
  ...
  Matrix eigenvectors() const {
    return SymGEigsSolver<Scalar, SelectionRule, OpType, BOpType,
                          GEIGS_CHOLESKY>::eigenvectors(this->m_nev);
  }
...
/// This solver requires two matrix operation objects: one for \f$A\f$ that
/// implements the matrix multiplication \f$Av\f$, and one for \f$B\f$ that
/// implements the matrix-vector product \f$Bv\f$ and the linear equation
/// solving operation \f$B^{-1}v\f$.
///
/// If \f$A\f$ and \f$B\f$ are stored as Eigen matrices, then the first
/// operation can be created using the DenseSymMatProd or SparseSymMatProd
/// classes, and the second operation can be created using the
/// SparseRegularInverse class. There is no wrapper class for a dense \f$B\f$
/// matrix since in this case the Cholesky mode is always preferred. If the
/// users need to define their own operation classes, then they should implement
/// all the public member functions as in those built-in classes.
///
/// \tparam Scalar        The element type of the matrix.
///                       Currently supported types are `float`, `double` and
///                       `long double`.
...
/// \tparam BOpType       The name of the matrix operation class for \f$B\f$.
/// Users could either
///                       use the wrapper class SparseRegularInverse, or define
///                       their own that implements all the public member
///                       functions as in SparseRegularInverse.
/// \tparam GEigsMode     Mode of the generalized eigen solver. In this solver
...
template <typename Scalar, int SelectionRule, typename OpType, typename BOpType>
class SymGEigsSolver<Scalar, SelectionRule, OpType, BOpType,
                     GEIGS_REGULAR_INVERSE>
    : public SymEigsBase<Scalar, SelectionRule,
                         SymGEigsRegInvOp<Scalar, OpType, BOpType>, BOpType> {
private:
  typedef Eigen::Index Index;
...
public:
  ...
  ///             implement the multiplication operation \f$Bv\f$ and the linear
  ///             equation solving operation \f$B^{-1}v\f$ for any vector
  ///             \f$v\f$. Users could either create the object from the wrapper
  ///             class SparseRegularInverse, or define their own that
  ///             implements all the public member functions as in
  ...
  SymGEigsSolver(OpType *op, BOpType *Bop, Index nev, Index ncv)
      : SymEigsBase<Scalar, SelectionRule,
                    SymGEigsRegInvOp<Scalar, OpType, BOpType>, BOpType>(
            new SymGEigsRegInvOp<Scalar, OpType, BOpType>(*op, *Bop), Bop, nev,
            ncv) {}
