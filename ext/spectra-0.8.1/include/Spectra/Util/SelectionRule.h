...
enum SELECT_EIGENVALUE {
  ...
  BOTH_ENDS ///< Select eigenvalues half from each end of the spectrum. When
            ///< `nev` is odd, compute more from the high end. Only for
            ///< symmetric eigen solvers.
};
...
/// \ingroup Enumerations
///
/// The enumeration of selection rules of desired eigenvalues. Alias for
/// `SELECT_EIGENVALUE`.
///
...
template <typename T> class ElemType<std::complex<T>> {
public:
  typedef T type;
};
...
template <typename Scalar, int SelectionRule> class SortingTarget {
public:
...
template <typename Scalar> class SortingTarget<Scalar, LARGEST_MAGN> {
public:
  static typename ElemType<Scalar>::type get(const Scalar &val) {
    using std::abs;
    return -abs(val);
...
public:
  static RealType get(const std::complex<RealType> &val) { return -val.real(); }
};
...
template <typename Scalar> class SortingTarget<Scalar, LARGEST_ALGE> {
public:
  static Scalar get(const Scalar &val) { return -val; }
};
...
template <typename Scalar> class SortingTarget<Scalar, BOTH_ENDS> {
public:
  static Scalar get(const Scalar &val) { return -val; }
};
...
template <typename Scalar> class SortingTarget<Scalar, SMALLEST_MAGN> {
public:
  static typename ElemType<Scalar>::type get(const Scalar &val) {
    using std::abs;
    return abs(val);
...
template <typename Scalar> class SortingTarget<Scalar, SMALLEST_ALGE> {
public:
  static Scalar get(const Scalar &val) { return val; }
};

// Sort eigenvalues and return the order index
template <typename PairType> class PairComparator {
public:
...
template <typename T, int SelectionRule> class SortEigenvalue {
private:
  typedef typename ElemType<T>::type
      TargetType; // Type of the sorting target, will be
                  // a floating number type, e.g. "double"
  typedef std::pair<TargetType, int>
      PairType; // Type of the sorting pair, including
                // the sorting target and the index

  std::vector<PairType> pair_sort;

public:
  SortEigenvalue(const T *start, int size) : pair_sort(size) {
