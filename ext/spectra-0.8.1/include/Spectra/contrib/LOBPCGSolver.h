...
///	*** METHOD
///	The class represent the LOBPCG algorithm, which was invented by Andrew
///Knyazev 	Theoretical background of the procedure can be found in the articles
///below
...
///on scientific computing, 23(2), pp.517 - 541.
///	- Knyazev, A.V., Argentati, M.E., Lashuk, I. and Ovtchinnikov, E.E.,
///2007. Block locally optimal preconditioned eigenvalue xolvers(BLOPEX) in
///HYPRE and PETSc.SIAM Journal on Scientific Computing, 29(5), pp.2224 - 2239.
///
///	*** CONDITIONS OF USE
///	Locally Optimal Block Preconditioned Conjugate Gradient(LOBPCG) is a
///method for finding the M smallest eigenvalues
/// and eigenvectors of a large symmetric positive definite generalized
/// eigenvalue problem
///	\f$Ax=\lambda Bx,\f$
///	where \f$A_{NxN}\f$ is a symmetric matrix, \f$B\f$ is symmetric and
///positive - definite. \f$A and B\f$ are also assumed large and sparse
///	\f$\textit{M}\f$ should be \f$\<< textit{N}\f$ (at least \f$\textit{5M}
///< \textit{N} \f$)
///
///	*** ARGUMENTS
///	Eigen::SparseMatrix<long double> A; // N*N - Ax = lambda*Bx, lrage and
///sparse 	Eigen::SparseMatrix<long double> X; // N*M - initial approximations to
///eigenvectors (random in general case) 	Spectra::LOBPCGSolver<long double>
///solver(A, X); 	*Eigen::SparseMatrix<long double> B; // N*N - Ax = lambda*Bx,
///sparse, positive definite 	solver.setConstraints(B); 	*Eigen::SparseMatrix<long
///double> Y; // N*K - constraints, already found eigenvectors 	solver.setB(B);
...
///	solver.residuals(); // get residuals to evaluate biases //
///Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> 	solver.eigenvalues();
///// get eigenvalues // Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>
///	solver.eigenvectors(); // get eigenvectors // Eigen::Matrix<Scalar,
///Eigen::Dynamic, 1>
///
///	*** EXAMPLE
/// \code{.cpp}
...
///	// random A
///	Matrix a;
///	a = (Matrix::Random(10, 10).array() > 0.6).cast<long double>() *
///Matrix::Random(10, 10).array() * 5; 	a =
///Matrix((a).triangularView<Eigen::Lower>()) +
///Matrix((a).triangularView<Eigen::Lower>()).transpose(); 	for (int i = 0; i <
///10; i++) 		a(i, i) = i + 0.5; 	std::cout << a << "\n"; 	Eigen::SparseMatrix<long
///double> A(a.sparseView());
///	// random X
...
template <typename Scalar = long double> class LOBPCGSolver {
private:
...
private:
  ...
  */
  ...
  Matrix stack_9_matricies(Matrix A, Matrix B, Matrix C, Matrix D, Matrix E,
                           Matrix F, Matrix G, Matrix H, Matrix I) {

    Matrix result(A.rows() + D.rows() + G.rows(),
    ...
    result.block(A.rows() + D.rows(), A.cols() + B.cols(), I.rows(), I.cols()) =
        I;
  ...
  void removeColumns(SparseMatrix &matrix, std::vector<int> &colToRemove) {
    ...
    SparseMatrix new_matrix(matrix.cols(),
                            matrix.cols() - int(colToRemove.size()));
    int iCol = 0;
    std::vector<Eigen::Triplet<Scalar>> tripletList;
    tripletList.reserve(matrix.cols() - int(colToRemove.size()));

    for (int iRow = 0; iRow < matrix.cols(); iRow++) {
      if (std::find(colToRemove.begin(), colToRemove.end(), iRow) ==
          colToRemove.end()) {
  ...
  int checkConvergence_getBlocksize(SparseMatrix &m_residuals,
                                    Scalar tolerance_L2,
...
public:
  LOBPCGSolver(const SparseMatrix &A, const SparseMatrix X)
      : m_n(A.rows()), m_nev(X.cols()), m_info(NOT_COMPUTED),
        flag_with_constraints(false), flag_with_B(false),
        flag_with_preconditioner(false), A(A), X(X) {
    if (A.rows() != X.rows() || A.rows() != A.cols())
      throw std::invalid_argument("Wrong size");

    // if (m_n < 5* m_nev)
    //	throw std::invalid_argument("The problem size is small compared to the
    //block size. Use standard eigensolver");
  }
  ...
  void compute(int maxit = 10, Scalar tol_div_n = 1e-7) {
    ...
    SparseMatrix directions, AX, AR, BX, AD, ADD, DD, BDD, BD, XAD, RAD, DAD,
        XBD, RBD, BR, sparse_eVecX, sparse_eVecR, sparse_eVecD, inverse_matrix;
    ...
    AX = A * X;
    // Solve the following NxN eigenvalue problem for all N eigenvalues and
    // -vectors: first approximation via a dense problem
    ...
    for (int iter_num = 0; iter_num < max_iter; iter_num++) {
      ...
      BlockSize = checkConvergence_getBlocksize(m_residuals, tolerance_L2,
                                                columnsToDelete);
      ...
      if (iter_num > 0) {
        ...
        gramA = stack_9_matricies(m_evalues.asDiagonal(), XAR, XAD,
                                  XAR.transpose(), RAR, RAD, XAD.transpose(),
                                  RAD.transpose(), DAD.transpose());
        gramB = stack_9_matricies(
            Matrix::Identity(m_nev, m_nev), XBR, XBD, XBR.transpose(),
            Matrix::Identity(BlockSize, BlockSize), RBD, XBD.transpose(),
            RBD.transpose(), Matrix::Identity(BlockSize, BlockSize));

      } else {
        gramA = stack_4_matricies(m_evalues.asDiagonal(), XAR, XAR.transpose(),
                                  RAR);
        gramB = stack_4_matricies(Matrix::Identity(m_nev, m_nev), XBR,
                                  XBR.transpose(),
                                  Matrix::Identity(BlockSize, BlockSize));
      }

      // calculate the lowest/largest m eigenpairs; Solve the generalized
      // eigenvalue problem.
  ...
  Vector eigenvalues() { return m_evalues; }

  Matrix eigenvectors() { return m_evectors; }

  Matrix residuals() { return Matrix(m_residuals); }
