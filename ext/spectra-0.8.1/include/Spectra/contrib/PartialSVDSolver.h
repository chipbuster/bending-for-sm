...
template <typename Scalar> class SVDMatOp {
public:
...
// MatrixType is either Eigen::Matrix<Scalar, ...> or
...
public:
  ...
  SVDTallMatOp(ConstGenericMatrix &mat)
      : m_mat(mat), m_dim(std::min(mat.rows(), mat.cols())),
        m_cache(mat.rows()) {}
...
public:
  ...
  PartialSVDSolver(ConstGenericMatrix &mat, int ncomp, int ncv)
    ...
    // Solver object
    m_eigs = new SymEigsSolver<Scalar, LARGEST_ALGE, SVDMatOp<Scalar>>(
        m_op, ncomp, ncv);
  }
