...
template <typename Scalar, typename ArnoldiOpType> class Arnoldi {
private:
...
protected:
  ...
  // Given orthonormal basis functions V, find a nonzero vector f such that V'Bf
  // = 0 Assume that f has been properly allocated
  void expand_basis(MapConstMat &V, const Index seed, Vector &f,
                    Scalar &fnorm) {
    using std::sqrt;
...
public:
  ...
  void init(MapConstVec &v0, Index &op_counter) {
    ...
    // it may contain tiny fluctuations. When this happens, we force f to be
    // zero
  ...
  virtual void factorize_from(Index from_k, Index to_m, Index &op_counter) {
    ...
    for (Index i = from_k; i <= to_m - 1; i++) {
      ...
      while (count < 5 && ortho_err > m_eps * m_beta) {
        ...
        // consists of noises of rounding errors, so the test [ortho_err < eps *
        // beta] is very likely to fail. In particular, if beta=0, then the test
        // is ensured to fail. Hence when this happens, we force f to be zero,
  ...
  void compress_V(const Matrix &Q) {
    ...
    Vector fk = m_fac_f * Q(m_m - 1, m_k - 1) +
                m_fac_V.col(m_k) * m_fac_H(m_k, m_k - 1);
    m_fac_f.swap(fk);
    m_beta = m_op.norm(m_fac_f);
  }
};
