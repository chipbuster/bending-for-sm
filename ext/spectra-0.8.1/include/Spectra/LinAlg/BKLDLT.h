...
// 2. Golub, G. H., & Van Loan, C. F. (2012). Matrix computations (Vol. 3). JHU
// press. Section 4.4.
// 3. Bunch-Parlett diagonal pivoting
// <http://oz.nthu.edu.tw/~d947207/Chap13_GE3.ppt>
template <typename Scalar = double> class BKLDLT {
private:
  ...
  IntVector m_perm; // [-2, -1, 3, 1, 4, 5]: 0 <-> 2, 1 <-> 1, 2 <-> 3, 3 <-> 1,
                    // 4 <-> 4, 5 <-> 5
  ...
  Scalar find_sigma(Index k, Index r, Index &p) {
    ...
    // First search A[r+1, r], ...,  A[end, r], which has the same task as
    // find_lambda() If r == end, we skip this search
    Scalar sigma = Scalar(-1);
    if (r < m_n - 1)
      sigma = find_lambda(r, p);

    // Then search A[k, r], ..., A[r-1, r], which maps to A[r, k], ..., A[r,
    // r-1]
    for (Index j = k; j < r; j++) {
