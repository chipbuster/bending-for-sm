...
template <typename Scalar = double> class DoubleShiftQR {
private:
  ...
  const Scalar
      m_near_0; // a very small value, but 1.0 / m_safe_min does not overflow
  ...
  void compute_reflector(const Scalar &x1, const Scalar &x2, const Scalar &x3,
                         Index ind) {
    using std::abs;
    ...
    Scalar x1_new =
        x1 - ((x1 <= 0) - (x1 > 0)) * Eigen::numext::hypot(x1, x2x3);
  ...
  void update_block(Index il, Index iu) {
    ...
    const Scalar x00 = m_mat_H.coeff(il, il), x01 = m_mat_H.coeff(il, il + 1),
    ...
    // m20 = x21 * x10
    const Scalar m20 =
        m_mat_H.coeff(il + 2, il + 1) * m_mat_H.coeff(il + 1, il);
    ...
    for (Index i = 1; i < bsize - 2; i++) {
      ...
      // Apply the reflector to X
      apply_PX(m_mat_H.block(il + i, il + i - 1, 3, m_n - il - i + 1), m_n,
               il + i);
      apply_XP(m_mat_H.block(0, il + i, il + std::min(bsize, Index(i + 4)), 3),
               m_n, il + i);
    }

    // The last reflector
    // This causes nr=2
    compute_reflector(m_mat_H.coeff(iu - 1, iu - 2), m_mat_H.coeff(iu, iu - 2),
                      0, iu - 1);
    // Apply the reflector to X
  ...
  void apply_PX(Scalar *x, Index u_ind) const {
    ...
    const Scalar dot2 =
        Scalar(2) * (x[0] * u0 + x[1] * u1 + (nr_is_2 ? 0 : (x[2] * u2)));
    x[0] -= dot2 * u0;
    x[1] -= dot2 * u1;
    if (!nr_is_2)
      x[2] -= dot2 * u2;
...
public:
  DoubleShiftQR(Index size)
      : m_n(size), m_near_0(TypeTraits<Scalar>::min() * Scalar(10)),
        m_eps(Eigen::NumTraits<Scalar>::epsilon()), m_eps_rel(m_eps),
        m_eps_abs(m_near_0 * (m_n / m_eps)), m_computed(false) {}

  DoubleShiftQR(ConstGenericMatrix &mat, const Scalar &s, const Scalar &t)
      : m_n(mat.rows()), m_mat_H(m_n, m_n), m_shift_s(s), m_shift_t(t),
        m_ref_u(3, m_n), m_ref_nr(m_n),
