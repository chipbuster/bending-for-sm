...
///
template <typename Scalar = double> class UpperHessenbergQR {
private:
...
protected:
  ...
  static void compute_rotation(const Scalar &x, const Scalar &y, Scalar &r,
                               Scalar &c, Scalar &s) {
    using std::sqrt;
...
public:
  ...
  UpperHessenbergQR(Index size)
      : m_n(size), m_rot_cos(m_n - 1), m_rot_sin(m_n - 1), m_computed(false) {}
  ...
  UpperHessenbergQR(ConstGenericMatrix &mat, const Scalar &shift = Scalar(0))
      : m_n(mat.rows()), m_shift(shift), m_rot_cos(m_n - 1), m_rot_sin(m_n - 1),
        m_computed(false) {
    compute(mat, shift);
  }
  ...
  virtual void compute(ConstGenericMatrix &mat,
    ...
    for (Index i = 0; i < n1; i++) {
      ...
      // m_mat_T.block(i + 1, i + 1, 1, m_n - i - 1) += s * mat_T.block(i, i +
      // 1, 1, m_n - i - 1);
    }
  ...
  ///
  /// \return Returned matrix type will be `Eigen::Matrix<Scalar, ...>`,
  ...
  ///
  /// Overwrite `dest` with \f$Q'HQ = RQ + sI\f$, where \f$H\f$ is the input
  /// matrix `mat`, and \f$s\f$ is the shift. The result is an upper Hessenberg
  /// matrix.
  ///
  /// \param mat The matrix to be overwritten, whose type should be
  /// `Eigen::Matrix<Scalar, ...>`, depending on the template parameter `Scalar`
  /// defined.
  ///
  virtual void matrix_QtHQ(Matrix &dest) const {
    ...
    for (Index i = 0; i < n1; i++) {
      ...
      dest.block(0, i, i + 2, 1)     = c * Yi - s * dest.block(0, i + 1, i + 2,
      1); dest.block(0, i + 1, i + 2, 1) = s * Yi + c * dest.block(0, i + 1, i +
      2, 1); */
    }
  ...
  /// Apply the \f$Q\f$ matrix to a vector \f$y\f$.
  ///
  /// \param Y A vector that will be overwritten by the matrix product
  /// \f$Q'y\f$.
  ///
  /// Vector type can be `Eigen::Vector<Scalar, ...>`, depending on
  /// the template parameter `Scalar` defined.
  ...
  /// Apply the \f$Q\f$ matrix to another matrix \f$Y\f$.
  ///
  /// \param Y A matrix that will be overwritten by the matrix product
  /// \f$Q'Y\f$.
  ///
  /// Matrix type can be `Eigen::Matrix<Scalar, ...>` (e.g.
  /// `Eigen::MatrixXd` and `Eigen::MatrixXf`), or its mapped version
...
public:
  ...
  /// be performed later by calling the compute() method.
  ///
  TridiagQR(Index size) : UpperHessenbergQR<Scalar>(size) {}
  ...
  ///
  /// Overwrite `dest` with \f$Q'HQ = RQ + sI\f$, where \f$H\f$ is the input
  /// matrix `mat`, and \f$s\f$ is the shift. The result is a tridiagonal
  /// matrix.
  ///
  /// \param mat The matrix to be overwritten, whose type should be
  /// `Eigen::Matrix<Scalar, ...>`, depending on the template parameter `Scalar`
  ...
  void matrix_QtHQ(Matrix &dest) const {
    ...
    for (Index i = 0; i < n1; i++) {
      ...
      const Scalar m11 = dest.coeff(i, i), m12 = m_T_usub.coeff(i),
