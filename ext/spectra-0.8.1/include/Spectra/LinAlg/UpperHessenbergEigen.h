...
template <typename Scalar = double> class UpperHessenbergEigen {
private:
        ...
        for (Index i = n - 2; i >= 0; i--) {
          ...
          if (m_eivalues.coeff(i).imag() < Scalar(0)) {
            ...
            if (m_eivalues.coeff(i).imag() == Scalar(0)) {
              ...
              Complex cc = Complex(x * lastra - lastw * ra + q * sa,
            ...
            // Overflow control
            Scalar t =
                std::max(abs(m_matT.coeff(i, n - 1)), abs(m_matT.coeff(i, n)));
            if ((eps * t) * t > Scalar(1))
...
public:
  UpperHessenbergEigen() : m_n(0), m_computed(false) {}
    ...
    while (i < m_n) {
      ...
      } else // Complex eigenvalues
      {
        ...
        m_eivalues.coeffRef(i + 1) =
            Complex(m_matT.coeff(i + 1, i + 1) + p, -z);
        i += 2;
      }
    }
  ...
  ComplexMatrix eigenvectors() {
    ...
    for (Index j = 0; j < n; ++j) {
      ...
      if (Eigen::numext::imag(m_eivalues.coeff(j)) == Scalar(0) || j + 1 == n) {
        ...
        for (Index i = 0; i < n; ++i) {
          matV.coeffRef(i, j) =
              Complex(m_eivec.coeff(i, j), m_eivec.coeff(i, j + 1));
          matV.coeffRef(i, j + 1) =
              Complex(m_eivec.coeff(i, j), -m_eivec.coeff(i, j + 1));
        }
        matV.col(j).normalize();
