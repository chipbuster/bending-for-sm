...
/// concrete eigen solvers such as GenEigsSolver and GenEigsRealShiftSolver.
///
...
private:
  ...
  const Scalar
      m_near_0; // a very small value, but 1.0 / m_near_0 does not overflow
  ...
  // Real Ritz values calculated from UpperHessenbergEigen have exact zero
  // imaginary part Complex Ritz values have exact conjugate pairs So we use
  // exact tests here
  ...
  void restart(Index k) {
    ...
    for (Index i = k; i < m_ncv; i++) {
      if (is_complex(m_ritz_val[i]) &&
          is_conj(m_ritz_val[i], m_ritz_val[i + 1])) {
        // H - mu * I = Q1 * R1
  ...
  void retrieve_ritzpair() {
    ...
    // Copy the Ritz values and vectors to m_ritz_val and m_ritz_vec,
    // respectively
...
public:
  ...
  GenEigsBase(OpType *op, BOpType *Bop, Index nev, Index ncv)
      : m_op(op), m_n(m_op->rows()), m_nev(nev), m_ncv(ncv > m_n ? m_n : ncv),
        m_nmatop(0), m_niter(0), m_fac(ArnoldiOpType(op, Bop), m_ncv),
        m_info(NOT_COMPUTED), m_near_0(TypeTraits<Scalar>::min() * Scalar(10)),
  ...
  ///                   (e.g. selecting the largest or smallest eigenvalues in
  ///                   the full spectrum) is specified by the template
  ///                   parameter `SelectionRule` of GenEigsSolver.
  ///
  ...
  /// Returned vector type will be `Eigen::Vector<std::complex<Scalar>, ...>`,
  ...
  ///
  ComplexMatrix eigenvectors() const { return eigenvectors(m_nev); }
};
