...
/// either
///                       use the DenseGenComplexShiftSolve wrapper class, or
///                       define their own that implements all the public member
///                       functions as in DenseGenComplexShiftSolve.
///
...
private:
  ...
  void sort_ritzpair(int sort_rule) {
    ...
    Vector v_real(this->m_n), v_imag(this->m_n), OPv_real(this->m_n),
        OPv_imag(this->m_n);
    const Scalar eps = Eigen::NumTraits<Scalar>::epsilon();
    for (Index i = 0; i < this->m_nev; i++) {
      v_real.noalias() =
          this->m_fac.matrix_V() * this->m_ritz_vec.col(i).real();
      v_imag.noalias() =
          this->m_fac.matrix_V() * this->m_ritz_vec.col(i).imag();
      ...
      const Complex root_part2 =
          Scalar(0.5) *
          sqrt(Scalar(1) - Scalar(4) * m_sigmai * m_sigmai * (nu * nu)) / nu;
    ...
    GenEigsBase<Scalar, SelectionRule, OpType, IdentityBOp>::sort_ritzpair(
        sort_rule);
  }
...
public:
  ...
  /// mode.
  ///
  /// \param op      Pointer to the matrix operation object. This class should
  /// implement
  ///                the complex shift-solve operation of \f$A\f$: calculating
  ///                \f$\mathrm{Re}\{(A-\sigma I)^{-1}v\}\f$ for any vector
  ///                \f$v\f$. Users could either create the object from the
  ///                DenseGenComplexShiftSolve wrapper class, or define their
  ///                own that implements all the public member functions as in
  ///                DenseGenComplexShiftSolve.
  /// \param nev     Number of eigenvalues requested. This should satisfy
  /// \f$1\le nev \le n-2\f$,
  ///                where \f$n\f$ is the size of matrix.
  /// \param ncv     Parameter that controls the convergence speed of the
  /// algorithm.
  ///                Typically a larger `ncv` means faster convergence, but it
  ///                may also result in greater memory use and more matrix
  ///                operations in each iteration. This parameter must satisfy
  ///                \f$nev+2 \le ncv \le n\f$, and is advised to take \f$ncv
  ///                \ge 2\cdot nev + 1\f$.
  /// \param sigmar  The real part of the shift.
  /// \param sigmai  The imaginary part of the shift.
  ///
  GenEigsComplexShiftSolver(OpType *op, Index nev, Index ncv,
                            const Scalar &sigmar, const Scalar &sigmai)
