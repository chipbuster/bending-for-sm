...
/// \ingroup MatOp
///
/// This class defines the complex shift-solve operation on a general real
/// matrix \f$A\f$, i.e., calculating \f$y=\mathrm{Re}\{(A-\sigma I)^{-1}x\}\f$
/// for any complex-valued \f$\sigma\f$ and real-valued vector \f$x\f$. It is
/// mainly used in the GenEigsComplexShiftSolver eigen solver.
///
template <typename Scalar> class DenseGenComplexShiftSolve {
private:
...
public:
  ...
  void set_shift(Scalar sigmar, Scalar sigmai) {
    m_solver.compute(m_mat.template cast<Complex>() -
                     Complex(sigmar, sigmai) *
                         ComplexMatrix::Identity(m_n, m_n));
