...
/// sparse positive definite matrix, \f$B=LL'\f$, where \f$L\f$ is a lower
/// triangular matrix. It is mainly used in the SymGEigsSolver generalized eigen
/// solver in the Cholesky decomposition mode.
///
template <typename Scalar, int Uplo = Eigen::Lower, int Flags = 0,
...
public:
  ...
  ///
  SparseCholesky(ConstGenericSparseMatrix &mat) : m_n(mat.rows()) {
    if (mat.rows() != mat.cols())
      throw std::invalid_argument("SparseCholesky: matrix must be square");

    m_decomp.compute(mat);
    m_info = (m_decomp.info() == Eigen::Success) ? SUCCESSFUL : NUMERICAL_ISSUE;
  }
