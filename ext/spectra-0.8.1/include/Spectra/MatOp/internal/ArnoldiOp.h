...
///
template <typename Scalar, typename OpType, typename BOpType> class ArnoldiOp {
private:
...
public:
  ...
  // In generalized eigenvalue problem Ax=lambda*Bx, define the inner product to
  // be <x, y> = x'By. For regular eigenvalue problems, it is the usual inner
  // product <x, y> = x'y
  ...
  template <typename Arg> Scalar norm(const Arg &x) {
    using std::sqrt;
...
public:
  ...
  // X is a matrix, y is a vector, res is a vector
  template <typename Arg1, typename Arg2>
  void trans_product(const Arg1 &x, const Arg2 &y,
                     Eigen::Ref<Vector> res) const {
    res.noalias() = x.transpose() * y;
  }

  // B-norm of a vector. For regular eigenvalue problems it is simply the L2
  // norm
  template <typename Arg> Scalar norm(const Arg &x) { return x.norm(); }
