...
void run_test(const SpMatrix &A, const SpMatrix &B, int k, int m,
              bool allow_fail = false) {
  typedef SparseSymMatProd<double> OpType;
  typedef SparseRegularInverse<double> BOpType;
  OpType op(A);
  BOpType Bop(B);
  SymGEigsSolver<double, SelectionRule, OpType, BOpType, GEIGS_REGULAR_INVERSE>
      eigs(&op, &Bop, k, m);
  eigs.init();
  int nconv =
...
void run_test_sets(const SpMatrix &A, const SpMatrix &B, int k, int m) {
  SECTION("Largest Magnitude") { run_test<LARGEST_MAGN>(A, B, k, m); }
  SECTION("Largest Value") { run_test<LARGEST_ALGE>(A, B, k, m); }
  SECTION("Smallest Magnitude") { run_test<SMALLEST_MAGN>(A, B, k, m, true); }
  SECTION("Smallest Value") { run_test<SMALLEST_ALGE>(A, B, k, m); }
  SECTION("Both Ends") { run_test<BOTH_ENDS>(A, B, k, m); }
}
