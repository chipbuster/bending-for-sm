...
template <typename MatType> void run_test(const MatType &mat, int k, int m) {
...
TEST_CASE("Partial SVD of tall sparse matrix [1000x100]",
          "[svds_sparse_tall]") {
  std::srand(123);
...
TEST_CASE("Partial SVD of wide sparse matrix [1000x100]",
          "[svds_sparse_wide]") {
  std::srand(123);
