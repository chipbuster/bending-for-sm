...
void run_test(const Matrix &mat, int k, int m, double sigmar, double sigmai,
  ...
  GenEigsComplexShiftSolver<double, SelectionRule,
                            DenseGenComplexShiftSolve<double>>
      eigs(&op, k, m, sigmar, sigmai);
  eigs.init();
  int nconv =
      eigs.compute(100); // maxit = 500 to reduce running time for failed cases
...
void run_test_sets(const Matrix &A, int k, int m, double sigmar,
                   double sigmai) {
