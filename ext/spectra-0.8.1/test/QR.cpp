...
void run_test(MatrixType &H, double shift) {
  ...
  INFO("Whether R is upper triangular, error = "
       << Rlower.cwiseAbs().maxCoeff());
  ...
  INFO(
      "max error of Q'Y = " << (QtY - Q.transpose() * Y).cwiseAbs().maxCoeff());
  REQUIRE((QtY - Q.transpose() * Y).cwiseAbs().maxCoeff() ==
          Approx(0.0).margin(tol));

  MatrixXd YQt = Y;
  decomp.apply_YQt(YQt);
  INFO(
      "max error of YQ' = " << (YQt - Y * Q.transpose()).cwiseAbs().maxCoeff());
  REQUIRE((YQt - Y * Q.transpose()).cwiseAbs().maxCoeff() ==
          Approx(0.0).margin(tol));
...
TEST_CASE("QR decomposition with double shifts", "[QR]") {
  ...
  // Equal up to signs
  INFO("max error of Q = "
       << (Q.cwiseAbs() - Q0.cwiseAbs()).cwiseAbs().maxCoeff());
  REQUIRE((Q.cwiseAbs() - Q0.cwiseAbs()).cwiseAbs().maxCoeff() ==
          Approx(0.0).margin(tol));

  // Test Q'HQ
  MatrixXd QtHQ;
  decomp.matrix_QtHQ(QtHQ);
  INFO("max error of Q'HQ = "
       << (QtHQ - Q.transpose() * H * Q).cwiseAbs().maxCoeff());
  REQUIRE((QtHQ - Q.transpose() * H * Q).cwiseAbs().maxCoeff() ==
          Approx(0.0).margin(tol));
