...
void run_test(const MatType &mat, int k, int m, bool allow_fail = false) {
  ...
  eigs.init();
  int nconv =
      eigs.compute(500); // maxit = 500 to reduce running time for failed cases
...
template <typename MatType> void run_test_sets(const MatType &A, int k, int m) {
  SECTION("Largest Magnitude") { run_test<MatType, LARGEST_MAGN>(A, k, m); }
  SECTION("Largest Real Part") { run_test<MatType, LARGEST_REAL>(A, k, m); }
  ...
  SECTION("Smallest Real Part") { run_test<MatType, SMALLEST_REAL>(A, k, m); }
