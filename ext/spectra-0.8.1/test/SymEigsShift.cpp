...
void run_test(const MatType &mat, int k, int m, double sigma,
  ...
  SymEigsShiftSolver<double, SelectionRule,
                     typename OpTypeTrait<MatType>::OpType>
      eigs(&op, k, m, sigma);
  eigs.init();
  int nconv =
      eigs.compute(150); // maxit = 150 to reduce running time for failed cases
...
void run_test_sets(const MatType &mat, int k, int m, double sigma) {
  ...
  SECTION("Both Ends") { run_test<MatType, BOTH_ENDS>(mat, k, m, sigma); }
}
