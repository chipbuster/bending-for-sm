...
void run_test(const MatType &mat, int k, int m) {
  ...
  Matrix resid = mat.template selfadjointView<Eigen::Lower>() * evecs -
                 evecs * evals.asDiagonal();
...
void run_test_sets(const MatType &mat, int k, int m) {
  SECTION("Largest Magnitude") { run_test<MatType, LARGEST_MAGN>(mat, k, m); }
  SECTION("Largest Value") { run_test<MatType, LARGEST_ALGE>(mat, k, m); }
  SECTION("Smallest Magnitude") { run_test<MatType, SMALLEST_MAGN>(mat, k, m); }
  SECTION("Smallest Value") { run_test<MatType, SMALLEST_ALGE>(mat, k, m); }
  SECTION("Both Ends") { run_test<MatType, BOTH_ENDS>(mat, k, m); }
}
...
TEST_CASE("Eigensolver of sparse symmetric real matrix [100x100]",
          "[eigs_sym]") {
  std::srand(123);
...
TEST_CASE("Eigensolver of sparse symmetric real matrix [1000x1000]",
          "[eigs_sym]") {
  std::srand(123);
