...
/// The destructor
/**
 * Virtual since GLVU has virtual methods and allocates some members on the
 * heap.
 */
GLVU::~GLVU() {
  delete[] Cams;
 ...
 *
 * @see InitWin */
int GLVU::Init(char *WindowTitle, unsigned int VisualMode, int WindowStartX,
 ...
 */
void GLVU::DefaultDisplayFunc() { GetGLVU()->Display(); }
 ...
 *     DefaultMouseFunc, DefaultMotionFunc
 */
void GLVU::DefaultReshapeFunc(int WW, int WH) { GetGLVU()->Reshape(WW, WH); }
...
void GLVU::PrintVisualInfo() {
  ...
  printf("Stencil: %d\n", i);
  glGetBooleanv(GL_DOUBLEBUFFER, &j);
  printf("Double Buffer?  %d\n", j);
  glGetBooleanv(GL_STEREO, &j);
  printf("Stereo Buffer?  %d\n", j);
...
// the text module. Access functions are provided. The following fonts are
// available and are defined in glut.h (fontids are on the left):
...
// @param LookAtCntr   starting world point to look towards (usually model
center)
...
*/
//----------------------------------------------------------------------------
void GLVU::AllCamsPerspectiveChange(float Yfov, float Aspect, float Ndist,
                                    float Fdist) {
  for (int i = 0; i < NumCams; i++)
 ...
 * OpenGL matrix stack, just calling BeginFrame() is a convenient and effective
 * way to set things up so you can just do your application-specific OpenGL
 * drawing.
 *
...
*/
//----------------------------------------------------------------------------
void GLVU::GetPixelRay(int sx, int sy, int ww, int wh, glvuVec3f *Start,
                       glvuVec3f *Dir) const {
...
void GLVU::TranslateX(int OldX, int NewX, int WW) {
  ...
  } else // FOR OUTSIDE-LOOKING-IN
  {
    ...
    if (DistToCntr < WorldRadius)
      DistToCntr = WorldRadius;
...
void GLVU::HyperBall(int OldX, int OldY, int NewX, int NewY, int WW, int WH) {
  ...
  Translate16fv(M, WorldCenter.x, WorldCenter.y, WorldCenter.z);
  invViewing16fv(N, &(Cam->X.x), &(Cam->Y.x), &(Cam->Z.x),
                 &(glvuVec3f::ZERO.x));
  Mult16fv(M, M, N);
  build_rotmatrix(Q, q);
...
void GLVU::StartRecording(const char *FileName) {
  ...
  if (!RecordPlaybackFP) {
    printf("ERROR (StartRecording) fp==NULL\n");
    return;
  }
  RecordingOn = 1;
}
...
void GLVU::StartPlayback(const char *FileName) {
  ...
  if (!RecordPlaybackFP) {
    printf("ERROR (StartPlayback) fp==NULL\n");
    return;
  }
  PlaybackOn = 1;
...
void GLVU::EndPlayback() {
  if (!PlaybackOn) {
    printf("Not playing back!\n");
    return;
  }
  if (RecordingOn) {
    printf("Recording is on! Not playing back!\n");
    return;
...
void GLVU::StopRecordingPlayback() {
  if (PlaybackOn)
    ...
    printf("Not recording or playing back!\n");
}
 ...
 *
 * @param x,y The position for the lower left corner of the text, relative to
...
void GLVU::Print(int x, int y, char *buffer) { Text(x, y, buffer); }
...
void GLVU::Keyboard(unsigned char Key, int x, int y) {
  ...
  case '=':
    vuOptionsMenuHandler(1);
    break;
  case 'o':
    vuOptionsMenuHandler(2);
    break;
  case 'r':
    vuOptionsMenuHandler(3);
    break;
  case 'i':
    vuOptionsMenuHandler(4);
    break;

  case 'w':
    glOptionsMenuHandler(2);
    break;
  case 'b':
    glOptionsMenuHandler(3);
    break;
  case 'l':
    glOptionsMenuHandler(8);
    break;
  case 'n':
    glOptionsMenuHandler(9);
    break;
  case 'm':
    glOptionsMenuHandler(10);
    break;

  case '1':
    CamViewMenuHandler(0);
    break;
  case '2':
    CamViewMenuHandler(1);
    break;
  case '3':
    CamViewMenuHandler(2);
    break;
  case '4':
    CamViewMenuHandler(3);
    break;

  case '!':
    CamViewDisplayMenuHandler(0);
    break;
  case '@':
    CamViewDisplayMenuHandler(1);
    break;
  case '#':
    CamViewDisplayMenuHandler(2);
    break;
  case '$':
    CamViewDisplayMenuHandler(3);
    break;
  };
}
...
void GLVU::vuOptionsMenuHandler(int value) {
  ...
  case 4: {
    GLVU *g = GetGLVU();
    if (g->GetInertiaEnabled()) {
      g->SetInertiaEnabled(0);
      g->SetInertiaOn(0);
    } else
      g->SetInertiaEnabled(1);
  } break;
  case 5:
...
void GLVU::CamViewDisplayMenuHandler(int value) {
  GLVU *g = GetGLVU();
  if (g->CamDisplayOn[value])
    g->CamDisplayOn[value] = 0;
  else
    g->CamDisplayOn[value] = 1;
  glutPostRedisplay();
}

void GLVU::EscapeHandler(int value) { exit(1); }
...
#ifndef ABS
template <class T> static bool ABS(T x) { return (((x) < 0) ? (-(x)) : x); }
#endif
...
void GLVU::Mouse(int button, int state, int x, int y) {
  ...
  if (button == GLUT_LEFT_BUTTON) {
    ...
    {
      OldX = x;
      OldY = y;                // RESET THE OLD TO THE CURRENT (starting over)
    ...
    } else if (LeftButtonDown) // LEFT-BUTTON UP after LEFT-BUTTON DOWN
    {
      ...
      // THE GLOBAL InertiaEnabled FLAG IS SET (CAN BE SET BY
      // SetInertiaEnabled).
 ...
 */
void GLVU::DefaultMotionFunc(int x, int y) { GetGLVU()->Motion(x, y); }
...
void GLVU::Motion(int x, int y) {
  ...
  if (LeftButtonDown) {
    ...
    case NAV_MODE_TRACKBALL:
      if (CtrlPressed) {
        TrackBallX(OldX, NewX, WW);
        DriveY(OldY, NewY, WH);
      } else {
        TrackBallX(OldX, NewX, WW);
        TrackBallY(OldY, NewY, WH);
      }
      break;
    case NAV_MODE_HYPERBALL:
      if (CtrlPressed) {
        HyperBall(OldX, OldY, NewX, OldY, WW, WH);
        DriveY(OldY, NewY, WH);
      } else {
        HyperBall(OldX, OldY, NewX, NewY, WW, WH);
      }
      break;
    case NAV_MODE_DRIVE:
      if (CtrlPressed) {
        TranslateX(NewX, OldX, WW);
        TranslateY(NewY, OldY, WH);
      } else if (AltPressed) {
        LookX(OldX, NewX, WW);
        LookY(OldY, NewY, WH);
      } else {
 ...
 *     DefaultMouseFunc, DefaultMotionFunc
 */
void GLVU::DefaultInertiaFunc(int x, int y) { GetGLVU()->Inertia(x, y); }
 ...
 */
void GLVU::Inertia(int x, int y) { GLVU::Motion(x, y); }
...
int GLVU::DoInertia() {
  if (InertiaOn) {
    // SIMPLY REPEATEDLY CALL THE MOUSE MOVE HANDLER (MUST SET APPROPRIATE
    // STATES)
    int tNewX = NewX, tNewY = NewY; // COPY NEW VALUES TO TEMPS
    NewX = OldX;
    NewY = OldY;           // COPY OLD TO NEW (MOUSE MOVE COPIES)
