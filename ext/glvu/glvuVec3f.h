 ...
 *
 */
template <class Type> class glvuVec3 {
public:
  ...
  friend inline glvuVec3 operator*(Type s,
                                   const glvuVec3 &v) /// SCALAR MULT s*V
  {
  ...
  glvuVec3 &
  operator-=(const glvuVec3 &A) /// ACCUMULATED VECTOR SUBTRACTION (-=)
  {
    x -= A.x;
    y -= A.y;
    z -= A.z;
  ...
  glvuVec3 &
  operator&=(const glvuVec3 &A) /// ACCUMULATED COMPONENT MULTIPLY (&=)
  {
    x *= A.x;
    y *= A.y;
    z *= A.z;
  ...
  /*
      const Type& operator [] (const int i) const // ALLOWS VECTOR ACCESS AS AN
     ARRAY. { return( (i==0)?x:((i==1)?y:z) ); }; Type & operator [] (const int
     i) { return( (i==0)?x:((i==1)?y:z) ); };
  */
  ...
  void Print() const { printf("(%.3f, %.3f, %.3f)\n", x, y, z); }
...
template <class Type>
glvuVec3<Type> glvuVec3<Type>::ZERO = glvuVec3<Type>(0, 0, 0);
