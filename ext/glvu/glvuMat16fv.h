...
inline float *Rotate16fv(float *M, float DegAng, const float Axis[3]) {
  ...
  float ca = (float)cos(RadAng), sa = (float)sin(RadAng);
...
inline float *LookAt(float *M, const float Eye[3], const float LookAtPt[3],
                     const float ViewUp[3]) {
  float X[3], Y[3], Z[3];
  Subtract3fv(Z, Eye, LookAtPt);
  Normalize3fv(Z);
  CrossProd3fv(X, ViewUp, Z);
  Normalize3fv(X);
  CrossProd3fv(Y, Z, X);
  Normalize3fv(Y);
  M[0] = X[0];
  M[4] = X[1];
  M[8] = X[2];
  M[12] = -DotProd3fv(X, Eye); // TRANS->ROT
  M[1] = Y[0];
  M[5] = Y[1];
  M[9] = Y[2];
  M[13] = -DotProd3fv(Y, Eye);
  M[2] = Z[0];
  M[6] = Z[1];
  M[10] = Z[2];
  M[14] = -DotProd3fv(Z, Eye);
  M[3] = 0;
  M[7] = 0;
  M[11] = 0;
  M[15] = 1;
...
inline float *invLookAt(float *M, const float Eye[3], const float LookAtPt[3],
                        const float ViewUp[3]) {
  float X[3], Y[3], Z[3];
  Subtract3fv(Z, Eye, LookAtPt);
  Normalize3fv(Z);
...
inline float *Frustum16fv(float *M, float l, float r, float b, float t, float n,
                          float f) {
  M[0] = (2 * n) / (r - l);
  M[4] = 0;
  M[8] = (r + l) / (r - l);
...
inline float *Perspective(float *M, float Yfov, float Aspect, float Ndist,
                          float Fdist) {
...
inline float *invPerspective(float *M, float Yfov, float Aspect, float Ndist,
                             float Fdist) {
  Yfov *= 0.0174532f; // CONVERT TO RADIANS
  float wT = (float)tan(Yfov * 0.5f) * Ndist, wB = -wT;
  float wR = wT * Aspect, wL = -wR;
...
inline float *Viewing16fv(float *M, const float X[3], const float Y[3],
                          const float Z[3], const float O[3]) {
  M[0] = X[0];
  M[4] = X[1];
  M[8] = X[2];
  M[12] = -DotProd3fv(X, O);
...
inline void Viewing2CoordFrame16fv(const float *M, float X[3], float Y[3],
                                   float Z[3], float O[3]) {
  X[0] = M[0];
  X[1] = M[4];
  X[2] = M[8];
  O[0] = -DotProd3fv(M, M + 12);
  Y[0] = M[1];
  Y[1] = M[5];
  Y[2] = M[9];
  O[1] = -DotProd3fv(M + 4, M + 12);
  Z[0] = M[2];
  Z[1] = M[6];
  Z[2] = M[10];
  O[2] = -DotProd3fv(M + 8, M + 12);
};

inline float *invViewing16fv(float *M, const float X[3], const float Y[3],
                             const float Z[3], const float O[3]) {
  M[0] = X[0];
  M[4] = Y[0];
  M[8] = Z[0];
...
inline float *PlanarReflection16fv(float M[16], const float P[4]) {
  float AA = P[0] * P[0], AB = P[0] * P[1], AC = P[0] * P[2], AD = P[0] * P[3],
        BB = P[1] * P[1], BC = P[1] * P[2], BD = P[1] * P[3], CC = P[2] * P[2],
        CD = P[2] * P[3];
  M[0] = 1 - 2 * AA;
  M[4] = -2 * AB;
  M[8] = -2 * AC;
...
//   C = Translation * Rotation * Scaling (using column
//   vectors/pre-multiplication) WorldPt = C * ModelPt
// The corresponding inverse Xform is also provided (GetWorld2ObjXform)
//--------------------------------------------------------------------------
inline float *Obj2WorldXform16fv(float *M, const float X[3], const float Y[3],
                                 const float Z[3], const float O[3],
                                 float Scale) {
...
inline float *World2ObjXform16fv(float *M, const float X[3], const float Y[3],
                                 const float Z[3], const float O[3],
                                 float Scale) {
  if (Scale <= 0) {
    printf("Too small scale!\n");
    Scale = 1;
  }
  ...
  M[4] = sX[1];
  M[8] = sX[2];
  M[12] = -DotProd3fv(O, sX);
  M[1] = sY[0];
  M[5] = sY[1];
  M[9] = sY[2];
  M[13] = -DotProd3fv(O, sY);
  M[2] = sZ[0];
  M[6] = sZ[1];
  M[10] = sZ[2];
  M[14] = -DotProd3fv(O, sZ);
  M[3] = 0;
  M[7] = 0;
  M[11] = 0;
  M[15] = 1;
...
inline float XformCoordFrame16fv(const float *M, float X[3], float Y[3],
                                 float Z[3], float O[3]) {
...
//----------------------------------------------------------------------------
inline float *Screen2WorldXform16fv(float *M, const float X[3],
                                    const float Y[3],
                                    const float Z[3], // VIEWING AXES
                                    const float O[3], // VIEWING ORIGIN
                                    float l, float r, float b, float t, float n,
                                    float f,        // PROJECTION
...
inline float *World2ScreenXform16fv(float *M, const float X[3],
                                    const float Y[3],
                                    const float Z[3], // VIEWING AXES
                                    const float O[3], // VIEWING ORIGIN
                                    float l, float r, float b, float t, float n,
