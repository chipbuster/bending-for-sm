...
public:
  ...
  Camera(const Camera &Cam) { Copy(Cam); };
  ...
  void LookAt(const glvuVec3f &Eye, const glvuVec3f &ViewRefPt,
              const glvuVec3f &ViewUp) {
    Z = Eye - ViewRefPt;
    Z.Normalize(); // CALC CAM AXES ("/" IS CROSS-PROD)
    X = ViewUp / Z;
    X.Normalize();
    Y = Z / X;
    Y.Normalize();
  ...
  void TightlyFitToSphere(const glvuVec3f &Eye, const glvuVec3f &ViewUp,
                          const glvuVec3f &Cntr, float Rad) {
    ...
    wT = (Near * Rad) / (float)sqrt(DistToCntr * DistToCntr -
                                    Rad * Rad); //(Near * Rad) / DistToCntr;
    wB = -wT;
    wL = wB;
    wR = wT;
  }
  void GetLookAtParams(glvuVec3f *Eye, glvuVec3f *ViewRefPt,
                       glvuVec3f *ViewUp) const {
    *Eye = Orig;
    *ViewRefPt = Orig - Z;
    *ViewUp = Y;
  }

  void GetPerspectiveParams(float *Yfov, float *Aspect, float *Ndist,
                            float *Fdist) const {
  ...
  void GetFrustumParams(float *l, float *r, float *b, float *t, float *Ndist,
                        float *Fdist) const {
    *l = wL;
    *r = wR;
    *b = wB;
  ...
  //----------------------------------------------------------------------------
  const glvuVec3f &wCOP() const { return (Orig); }

  //----------------------------------------------------------------------------
  // RETURNS THE VIEWING DIRECTION
  //----------------------------------------------------------------------------
  glvuVec3f ViewDir() const { return (-Z); }

  glvuVec3f ViewDirOffAxis() const {
    float x = (wL + wR) * 0.5f,
          y = (wT + wB) * 0.5f; // MIDPOINT ON VIEWPLANE WINDOW
  ...
  //----------------------------------------------------------------------------
  void Translate(const glvuVec3f &trans) { Orig += trans; }
  ...
  float *GetXform_Screen2Obj(float *M, int WW, int WH) const {
    Screen2WorldXform16fv(M, &(X.x), &(Y.x), &(Z.x), &(Orig.x), wL, wR, wB, wT,
                          Near, Far, WW, WH);
    return (M);
  }

  float *GetXform_Obj2Screen(float *M, int WW, int WH) const {
    World2ScreenXform16fv(M, &(X.x), &(Y.x), &(Z.x), &(Orig.x), wL, wR, wB, wT,
                          Near, Far, WW, WH);
    return (M);
  }
  ...
  //----------------------------------------------------------------------------
  void GetPixelRay(float sx, float sy, int ww, int wh, glvuVec3f *Start,
                   glvuVec3f *Dir) const {
  ...
  void WriteToFile(FILE *fp) const {
    if (fp == NULL) {
      printf("ERROR WRITING CAM TO FILE!\n");
      return;
    }
    fprintf(fp, "%f %f %f %f %f %f %f %f %f %f %f %f\n", X.x, X.y, X.z, Y.x,
            Y.y, Y.z, Z.x, Z.y, Z.z, Orig.x, Orig.y, Orig.z);
  }

  int ReadFromFile(FILE *fp) // RETURNS "1" IF SUCCESSFUL, "0" IF EOF
  {
    int Cond =
        fscanf(fp, "%f %f %f %f %f %f %f %f %f %f %f %f", &X.x, &X.y, &X.z,
               &Y.x, &Y.y, &Y.z, &Z.x, &Z.y, &Z.z, &Orig.x, &Orig.y, &Orig.z);
    return (Cond != EOF);
  }
  ...
  {
    // WINDOW EXTENTS ARE DEFINED ON THE NEAR PLANE, CALC NEAR PTS (IN CAM
    // COORDS)
    float NearZ = -Near;
    V[0].Set(wR, wT, NearZ);
    V[1].Set(wL, wT, NearZ);
