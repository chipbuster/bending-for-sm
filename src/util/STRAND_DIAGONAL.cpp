...
STRAND_DIAGONAL::STRAND_DIAGONAL(const SPARSE_MATRIX &A,
                                 const VECTORI &strandEnds)
...
#pragma omp for schedule(static)
  ...
  for (int x = 0; x < _totalStrands; x++) {
    if (_blockInverses[x].info() != Eigen::Success) {
      cout << " Block inverse failed on strand " << x
...
STRAND_DIAGONAL::~STRAND_DIAGONAL() { delete[] _blockInverses; }
...
#pragma omp for schedule(static)
  for (int x = 0; x < _totalStrands; x++) {
    ...
    // result.segment(start, end) = _blockInverses[x].solve(rhs.segment(start,
    // end));
