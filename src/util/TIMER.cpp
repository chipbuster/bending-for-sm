...
void TIMER::printTimings() {
  ...
  for (forwardIter = _timings.begin(); forwardIter != _timings.end();
       forwardIter++) {
  ...
  for (backwardIter = inverseMap.rbegin(); backwardIter != inverseMap.rend();
       backwardIter++) {
    ...
    //  cout << "[" << timing.c_str() << "]: "
    //       << name.c_str() << " " << hours(time) << ":" << minutes(time) <<
    //       ":" << seconds(time) << "s" << endl;
    cout << "[" << timing.c_str() << "]: " << name.c_str() << " " << hours(time)
         << ":" << minutes(time) << ":" << seconds(time) << ":"
         << (int(time * 1000) % 1000) << "ms" << endl;
  }
  cout << "===================================================================="
          "================="
       << endl;
...
void TIMER::printTimingsPerFrame(const int frames) {
  ...
  for (backwardIter = inverseMap.rbegin(); backwardIter != inverseMap.rend();
    ...
    cout << "[" << timeString.c_str() << "%]: " << name.c_str() << " "
         << hoursString.c_str() << ":" << minutesString << ":" << secondsString
         << " s total ";
    ...
    secondsString = string(buffer);
    cout << " " << hoursString.c_str() << ":" << minutesString.c_str() << ":"
         << secondsString.c_str() << " s/frame" << endl;
  }
  ...
  cout << "===================================================================="
          "=========================="
       << endl;
  cout << " Total time: " << hoursString.c_str() << ":" << minutesString.c_str()
       << ":" << secondsString.c_str() << " (" << totalTime << "s) " << endl;
