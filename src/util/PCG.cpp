...
PCG::PCG(const SPARSE_MATRIX &A, const DIAGONAL &diagonal)
    : _A(A), _diagonal(diagonal),
      //_tolerance(1e-9),
      _tolerance(1e-6),
      //_maxIterations(10000),
      _maxIterations(1000), _x0(A.rows()) {
  _x0.setZero();
}
...
VECTOR PCG::solveAMGCL(const VECTOR &rhs) {
  ...
  */
  ...
  for (; iter < maxiter && res_norm > eps; ++iter) {
    ...
    // if (prm.verbose)
    // std::cout << "iteration: " << iter << "\t" << std::scientific << res_norm
    // / norm_rhs << std::endl;
    std::cout << "iteration: " << iter << "\t" << std::scientific << res_norm
              << std::endl;
  }
...
VECTOR PCG::solveEigenStyle(const VECTOR &rhs) {
  ...
  const REAL threshold =
      Eigen::numext::maxi(_tolerance * _tolerance * rhsNorm2, considerAsZero);
  REAL residualNorm2 = residual.squaredNorm();
  ...
  VECTOR z = p;
  VECTOR tmp(p.size());
  REAL rdotzNew = Eigen::numext::real(
  ...
  while (_iterations < _maxIterations) {
    ...
    if (verbose) {
      ...
      std::cout << " PCG residual " << _iterations << ": "
                << residual.norm() / rhs.norm() << std::endl;
      // std::cout << " residual " << _iterations << ": " << residualNorm2 <<
      // std::endl; std::cout << " residual: " << residual.transpose() <<
      // std::endl;
    }
...
VECTOR PCG::solvePCR(const VECTOR &rhs) {
  ...
  const REAL considerAsZero = (std::numeric_limits<REAL>::min)();
  const REAL threshold =
      Eigen::numext::maxi(_tolerance * _tolerance * rhs.norm(), considerAsZero);

  // VECTOR p = _diagonal.apply(r);
  VECTOR p = r;

  VECTOR z(p.size());
  // REAL absNew = Eigen::numext::real(residual.dot(p));  // the square of the
  // absolute value of r scaled by invM
  VECTOR Ar = _A * r;
  VECTOR Ap = _A * p;
  REAL rAr = r.dot(Ar);
  while (_iterations < _maxIterations) {
    ...
    if (verbose) {
      ...
      std::cout << " residual " << _iterations << ": " << r.norm() / rhs.norm()
                << std::endl;
      // std::cout << " residual " << _iterations << ": " << residualNorm2 <<
      // std::endl;
    }
