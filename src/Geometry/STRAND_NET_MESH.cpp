...
REAL STRAND_NET_MESH::clamp(const REAL &input, const REAL &bottom,
                            const REAL &top) {
...
void STRAND_NET_MESH::initialize() {
  ...
  for (unsigned int y = 0; y < _totalStrands; y++) {
    ...
    for (unsigned int x = 1; x < strand.size(); x++) {
      ...
      _vertexToEdge[strand[x - 1]].push_back(
          VECTOR2I(strand[x], _edgeIndices.size() - 1));
      _vertexToEdge[strand[x]].push_back(
          VECTOR2I(strand[x - 1], _edgeIndices.size() - 1));
    }
  ...
  for (unsigned int x = 0; x < _totalBends; x++) {
    ...
    _restThetas[x] =
        acos(clamp(e0.dot(e1) / (e0.norm() * e1.norm()), -1.0, 1.0));
  }
...
VECTOR STRAND_NET_MESH::buildPerBendVector(
    const vector<VECTOR9> &perBendForces) const {
...
SPARSE_MATRIX STRAND_NET_MESH::buildPerBendMatrix(
    const vector<MATRIX9> &perBendHessians) const {
...
VECTOR STRAND_NET_MESH::buildPerEdgeVector(
...
SPARSE_MATRIX STRAND_NET_MESH::buildPerEdgeMatrix(
...
// stretching
VECTOR STRAND_NET_MESH::computeStretchingForces(
...
SPARSE_MATRIX STRAND_NET_MESH::computeStretchingHessian(
    const STRAND::STRETCHING &stretching) const {
...
SPARSE_MATRIX STRAND_NET_MESH::computeStretchingClampedHessian(
    const STRAND::STRETCHING &stretching) const {
  TIMER functionTimer(__FUNCTION__);
  vector<MATRIX6> perEdgeHessians(_totalEdges);
  for (unsigned int i = 0; i < _totalEdges; i++) {
...
SPARSE_MATRIX STRAND_NET_MESH::computeBendingHessian() {
  ...
  for (unsigned int x = 0; x < _totalBends; x++) {
    ...
    MATRIX3x2 deformation;
    deformation << e0, e1;
    const MATRIX9 hes = -len * _pFpx.transpose() *
                        bendMaterial.hessian(deformation, _restThetas[x]) *
                        _pFpx;
...
SPARSE_MATRIX STRAND_NET_MESH::computeBendingClampedHessian() {
  ...
  for (unsigned int x = 0; x < _totalBends; x++) {
    ...
    MATRIX9 hesOrig = _pFpx.transpose() *
                      bendMaterial.hessian(deformation, _restThetas[x]) * _pFpx;
    hes = -len * clampEigenvalues(hesOrig);
#else
    hes = -len * _pFpx.transpose() *
          bendMaterial.clampedHessian(deformation, _restThetas[x]) * _pFpx;
#endif
    perBendHessians[x] = hes;
  }
  functionTimer.stop();
