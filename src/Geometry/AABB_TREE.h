...
struct AABB_NODE {
  AABB_NODE(const vector<int> &inputPrimitiveIndices, const VECTOR3 &inputMins,
            const VECTOR3 &inputMaxs, const int &inputDepth)
...
/////////////////////////////////////////////////////////////////////////////////////////////
// Collision detection structure for vertex-triangle and edge-edge collision
// detection
//
// Nothing fancy here, just a search tree that gets the job done. There are
// online libarries that do something similar, but nothing that did quite what I
// want. Right now collision detection is consuming 90% of the running time, so
// anything is better than nothing.
/////////////////////////////////////////////////////////////////////////////////////////////
class AABB_TREE {
public:
  AABB_TREE(const vector<VECTOR3> &vertices,
            const vector<VECTOR3I> *surfaceTriangles);
  AABB_TREE(const vector<VECTOR3> &vertices,
            const vector<VECTOR2I> *surfaceEdges);
  ~AABB_TREE();

  // return a list of potential triangles nearby a vertex, subject to a distance
  // threshold
  void nearbyTriangles(const VECTOR3 &vertex, const REAL &eps,
                       vector<int> &faces) const;

  // return a list of potential triangles nearby an edge, subject to a distance
  // threshold
  void nearbyTriangles(const VECTOR2I &edge, const REAL &eps,
                       vector<int> &faces) const;

  // return a list of potential nearby edges, subject to a distance threshold
  void nearbyEdges(const VECTOR2I &edge, const REAL &eps,
                   vector<int> &faces) const;

  // return a list of potential nearby edges, subject to a distance threshold
  // here for debugging purposes, similar to the edge-based one, but doesn't
  // need to reference points in _vertices
  void nearbyTriangles(const VECTOR3 &mins, const VECTOR3 &maxs,
...
private:
  ...
  // bounding box
  void findEdgeBoundingBox(const vector<int> &edgeIndices, VECTOR3 &mins,
                           VECTOR3 &maxs) const;
  ...
  // return a list of potential triangles nearby a vertex, subject to a distance
  // threshold
  void nearbyEdges(const AABB_NODE *node, const VECTOR2I &edge, const REAL &eps,
  ...
  void nearbyTriangles(const AABB_NODE *node, const VECTOR3 &mins,
                       const VECTOR3 &maxs, const REAL &eps,
                       vector<int> &faces) const;

  // are we inside this AABB, subject to the distance eps?
  bool insideAABB(const AABB_NODE *node, const VECTOR3 &vertex,
  ...
  bool overlappingAABBs(const AABB_NODE *node, const VECTOR2I &edge,
                        const REAL &eps) const;
