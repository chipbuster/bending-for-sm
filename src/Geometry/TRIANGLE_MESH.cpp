...
TRIANGLE_MESH::TRIANGLE_MESH(const vector<VECTOR3> &restVertices,
    ...
    : _vertices(restVertices), _verticesOld(restVertices),
      _restVertices(restVertices), _triangles(triangles) {
  /*
  ...
  */
  ...
  //_vertexFaceEnergy = new VOLUME::VERTEX_FACE_COLLISION(stiffness,
  //_collisionEps); // default _vertexFaceEnergy = new
  // VOLUME::MCADAMS_COLLISION(stiffness, _collisionEps);

  _vertexFaceEnergy =
      new VOLUME::VERTEX_FACE_COLLISION(stiffness, _collisionEps);
...
// FOR NOW: See top of file to change which edge energy
  ...
  // verified on the bunny drop scene
  //_vertexFaceEnergy = new VOLUME::MCADAMS_COLLISION(stiffness, _collisionEps);
  //_edgeEdgeEnergy = new VOLUME::EDGE_HYBRID_COLLISION(stiffness,
  //_collisionEps);
}
...
vector<MATRIX2> TRIANGLE_MESH::computeDmInvs() {
  ...
  for (unsigned int f = 0; f < _triangles.size(); f++) {
    ...
    if (fabs(Rm.determinant()) > eps)
      ...
      Eigen::JacobiSVD<MATRIX2> svd(Rm,
      ...
      RmInv =
          svd.matrixV() * singularInvs.asDiagonal() * svd.matrixU().transpose();
    }
    MATRIX2 DmInv = RmInv;
    DmInvs[f] = DmInv;
  }
...
void TRIANGLE_MESH::computeSurfaceAreas() {
  ...
  for (unsigned int x = 0; x < _triangles.size(); x++) {
    // build each edge
    for (int y = 0; y < 3; y++) {
      pair<int, int> edge(_triangles[x][y], _triangles[x][(y + 1) % 3]);
...
///////////////////////////////////////////////////////////////////////
REAL TRIANGLE_MESH::computeStretchingEnergy(
    const SHELL::STRETCHING &stretching) const {
...
///////////////////////////////////////////////////////////////////////
VECTOR TRIANGLE_MESH::computeStretchingForces(
    const SHELL::STRETCHING &stretching) const {
  TIMER functionTimer(__FUNCTION__);
...
///////////////////////////////////////////////////////////////////////
VECTOR
TRIANGLE_MESH::computeBendingForces(const SHELL::BENDING &bending) const {
  TIMER functionTimer(__FUNCTION__);
  ...
  for (unsigned int index = 0; index < _flaps.size(); index++) {
    ...
    const VECTOR12 force =
        -_restFlapAreas[index] * bending.gradient(flap, theta);
...
///////////////////////////////////////////////////////////////////////
SPARSE_MATRIX
TRIANGLE_MESH::computeBendingHessian(const SHELL::BENDING &bending) const {
  TIMER functionTimer(__FUNCTION__);
...
// this is all super-slow, should be optimized
///////////////////////////////////////////////////////////////////////
SPARSE_MATRIX TRIANGLE_MESH::computeBendingClampedHessian(
    const SHELL::BENDING &bending) const {
  TIMER functionTimer(__FUNCTION__);
  vector<MATRIX12> perFlapHessians(_flaps.size());
  // #pragma omp parallel
  // #pragma omp for schedule(static)
  for (unsigned int i = 0; i < _flaps.size(); i++) {
    ...
    const REAL theta = _restThetas[i];
    const MATRIX12 hessian =
        -_restFlapAreas[i] * bending.clampedHessian(flap, theta);
    perFlapHessians[i] = hessian;
  }
...
// this is all super-slow, should be optimized
///////////////////////////////////////////////////////////////////////
SPARSE_MATRIX TRIANGLE_MESH::computeStretchingHessian(
    const SHELL::STRETCHING &stretching) const {
...
// this is all super-slow, should be optimized
///////////////////////////////////////////////////////////////////////
SPARSE_MATRIX TRIANGLE_MESH::computeStretchingClampedHessian(
    const SHELL::STRETCHING &stretching) const {
  vector<MATRIX9> perElementHessians(_triangles.size());
  for (unsigned int i = 0; i < _triangles.size(); i++) {
    const MATRIX3x2 &F = _Fs[i];
    ...
    const MATRIX6 hessian =
        -_restTriangleAreas[i] * stretching.clampedHessian(F);
    perElementHessians[i] = (pFpx.transpose() * hessian) * pFpx;
  }
...
bool TRIANGLE_MESH::readObjFile(const string &filename,
  ...
  bool success = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err,
                                  filename.c_str(), NULL, triangulate);
...
///////////////////////////////////////////////////////////////////////
bool TRIANGLE_MESH::writeObjFile(const string &filename,
                                 const TRIANGLE_MESH &triMesh,
  ...
  const vector<VECTOR3> &vertices =
      (restVertices) ? triMesh.restVertices() : triMesh.vertices();
...
///////////////////////////////////////////////////////////////////////
vector<VECTOR3>
TRIANGLE_MESH::normalizeVertices(const vector<VECTOR3> &vertices) {
...
///////////////////////////////////////////////////////////////////////
bool TRIANGLE_MESH::pointProjectsInsideTriangle(const VECTOR3 &v0,
                                                const VECTOR3 &v1,
                                                const VECTOR3 &v2,
                                                const VECTOR3 &v) {
...
REAL TRIANGLE_MESH::pointTriangleDistance(const VECTOR3 &v0, const VECTOR3 &v1,
  ...
  const VECTOR3 vertexDistances((v - v0).norm(), (v - v1).norm(),
                                (v - v2).norm());
...
///////////////////////////////////////////////////////////////////////
bool TRIANGLE_MESH::insideCollisionCell(const int triangleID,
...
// ///////////////////////////////////////////////////////////////////////
// REAL TRIANGLE_MESH::distanceToCollisionCellWall(const int triangleID, const
// VECTOR3& vertex)
// {
//   const VECTOR3I& t = _triangles[triangleID];
//   vector<VECTOR3> v;
...
//   {
//     VECTOR3 nebHat;
//     // inside check is pretty different if considering orphaned or unorphaned
//     edge if (neighbors[x] < 0){
//       int j = -neighbors[x] -1;
//       VECTOR3 p0 = (v[(j+1)%3] + v[j])/2;
//       VECTOR3 p1 = v[(j+2)%3];
...
void TRIANGLE_MESH::computeVertexFaceCollisions() {
  ...
  for (unsigned int x = 0; x < _vertices.size(); x++) {
    ...
    for (unsigned int y = 0; y < _triangles.size(); y++) {
      // if the surface triangle is so small the normal could be degenerate,
      // skip it
      ...
      if (_invertedVertices[t[0]] && _invertedVertices[t[1]] &&
          _invertedVertices[t[2]])
        continue;
      ...
      const REAL distance = pointTriangleDistance(
          _vertices[t[0]], _vertices[t[1]], _vertices[t[2]], surfaceVertex);
  ...
  if (_vertexFaceCollisions.size() > 0) {
    cout << " Found " << _vertexFaceCollisions.size()
         << " vertex-face collisions " << endl;
    cout << " pairs: " << endl;
  }
...
void TRIANGLE_MESH::computeVertexFaceCCD() {
  ...
  for (unsigned int x = 0; x < _vertices.size(); x++) {
    ...
    for (unsigned int y = 0; y < _triangles.size(); y++) {
      ...
      // Do the CCD test: If CCD succeeds, save the vertex index and the
      // triangle index
      ...
      if (_ccd->Vertex_Triangle_CCD(x00, x01, x10, x11, x20, x21, x30, x31,
                                    tHit)) {
  ...
  for (unsigned int x = 0; x < _vertexFaceCCDs.size(); x++) {
    ...
    cout << "(" << collision.first << ", " << collision.second.first << ", "
         << collision.second.second << ")" << endl;
  }
#endif
#endif
  return;
...
void TRIANGLE_MESH::computeEdgeEdgeCollisions() {
  ...
  for (unsigned int x = 0; x < _edges.size(); x++) {
    ...
    const REAL collisionEps = (eeDistance < _collisionEps) ? eeDistance :
    _collisionEps;
    */
    ...
    for (int j = 0; j < 2; j++) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (_insideSurfaceVertexOneRing.find(lookup) !=
            _insideSurfaceVertexOneRing.end())
          insideOneRing = true;
      }
    }
    if (insideOneRing)
    ...
    if (_insideSurfaceVertexOneRing.find(lookup) !=
    _insideSurfaceVertexOneRing.end()) continue; lookup.first = outerEdge[0];
    lookup.second = innerEdge[1];
    if (_insideSurfaceVertexOneRing.find(lookup) !=
    _insideSurfaceVertexOneRing.end()) continue; lookup.first = outerEdge[1];
    lookup.second = innerEdge[0];
    if (_insideSurfaceVertexOneRing.find(lookup) !=
    _insideSurfaceVertexOneRing.end()) continue; lookup.first = outerEdge[1];
    lookup.second = innerEdge[1];
    if (_insideSurfaceVertexOneRing.find(lookup) !=
    _insideSurfaceVertexOneRing.end()) continue;
    */
    ...
    if (closestDistance < _collisionEps) {
      ...
      // if there's another triangle on the other side (this is in case we're
      // looking at cloth) then store that one too
      ...
      if (triangle0.size() > 0)
        penetrating = faceEdgeIntersection(triangle0, edge);
      if (triangle1.size() > 0)
        penetrating = penetrating || faceEdgeIntersection(triangle1, edge);

      _edgeEdgeIntersections.push_back(penetrating);
      //_edgeEdgeCollisionEps.push_back(_collisionEps);

      // TODO: for completeness, should probably test the other edges against
      // the other pair, just in case we're looking at a degenerate case. In
      // general, seems redundant.
    }
  }
  ...
  if (_edgeEdgeCollisions.size() > 0) {
    cout << " Collision area array size: " << _edgeEdgeCollisionAreas.size()
         << endl;
    cout << " Intersections array size: " << _edgeEdgeIntersections.size()
         << endl;
    cout << " Found " << _edgeEdgeCollisions.size() << " edge-edge collisions "
         << endl;
    cout << " pairs: " << endl;
  }
...
void TRIANGLE_MESH::gatherCCDRegions() {
  ...
  for (unsigned int i = 0; i < _allCCDs.size(); i++) {
    ...
    for (unsigned int j = 0; j < _vertexRegionsCCD.size(); j++) {
      ...
      // If so, add its info to the region vertices (but not yet with the region
      // data)
      staysAlong.push_back(true);
      if (sharedVerts.size() != 0) {
        accumulator.insert(_vertexRegionsCCD[j].begin(),
                           _vertexRegionsCCD[j].end());
    ...
    if (accumulator.size() == 0) {
      ...
      for (unsigned int k = 0; k < _vertexRegionsCCD.size(); k++) {
        if (staysAlong[k])
          regionReplace.push_back(_vertexRegionsCCD[k]);
      }
  ...
  // SECOND LOOP: All the regions are created and work contiguously; now just
  // add the corresponding CCDs to their regions
  for (unsigned int i = 0; i < _allCCDs.size(); i++) {
    // The ccd in question
    pair<REAL, VECTOR5I> ccd = _allCCDs[i];
    // the set of vertices that the collision represents
  ...
  if (_regionsCCD.size() > 0) {
    ...
    cout << "_regionsCCD.size(): " << _regionsCCD.size()
         << " | _vertexRegionsCCD.size(): " << _vertexRegionsCCD.size() << endl;
...
///////////////////////////////////////////////////////////////////////
VECTOR3 TRIANGLE_MESH::pointPlaneProjection(const vector<VECTOR3> &plane,
                                            const VECTOR3 &point) {
...
VECTOR TRIANGLE_MESH::computeVertexFaceCollisionForces() const {
  ...
  for (unsigned int i = 0; i < _vertexFaceCollisionTets.size(); i++) {
    ...
    const VECTOR12 force =
        -_vertexFaceCollisionAreas[i] * _vertexFaceEnergy->gradient(vs);
...
VECTOR TRIANGLE_MESH::computeEdgeEdgeCollisionForces() const {
  ...
  for (unsigned int i = 0; i < _edgeEdgeCollisions.size(); i++) {
    ...
    const VECTOR12 force =
        (!intersect) ? -area * _edgeEdgeEnergy->gradient(vs, a, b)
...
#else
    const MATRIX12 H =
        (!_edgeEdgeIntersections[i])
            ? -_edgeEdgeCollisionAreas[i] *
                  _edgeEdgeEnergy->clampedHessian(vs, a, b)
            : -_edgeEdgeCollisionAreas[i] *
                  _edgeEdgeEnergy->clampedHessianNegated(vs, a, b);
#endif
...
SPARSE_MATRIX TRIANGLE_MESH::computeVertexFaceCollisionClampedHessian() const {
  ...
  for (unsigned int i = 0; i < _vertexFaceCollisionTets.size(); i++) {
    ...
    for (unsigned int j = 0; j < 4; j++)
      vs[j] = _vertices[_vertexFaceCollisionTets[i][j]];
    const MATRIX12 H =
        -_vertexFaceCollisionAreas[i] * _vertexFaceEnergy->clampedHessian(vs);
    perElementHessians[i] = H;
  }
...
///////////////////////////////////////////////////////////////////////
VECTOR4I TRIANGLE_MESH::buildFlap(const int triangle0,
                                  const int triangle1) const {
...
void TRIANGLE_MESH::computeFlaps() {
  ...
  for (unsigned int x = 0; x < _edgeTriangleNeighbors.size(); x++) {
    ...
    if (edgeTriangles[0] == -1 || edgeTriangles[1] == -1)
      continue;

    const VECTOR4I flap = buildFlap(edgeTriangles[0], edgeTriangles[1]);
