...
public:
  ...
  /*
  const vector<VECTOR4I>& vertexFaceCollisionTets() const    { return
  _vertexFaceCollisionTets; }; vector<VECTOR4I>& vertexFaceCollisionTets() {
  return _vertexFaceCollisionTets; };
  */
  ...
  const VECTOR3 &vertexOld(const int index) const {
    return _verticesOld[index];
  };
  VECTOR3 &vertexOld(const int index) { return _verticesOld[index]; };
  const VECTOR3 &restVertex(const int index) const {
    return _restVertices[index];
  };
  const REAL &collisionEps() const { return _collisionEps; };
  const vector<bool> &invertedVertices() const { return _invertedVertices; };
  const vector<VECTOR2I> &edgeTriangleNeighbors() const {
    return _edgeTriangleNeighbors;
  };
  /*
  ...
  const vector<pair<int,int> >& vertexFaceCollisions() const { return
  _vertexFaceCollisions; }; const vector<pair<int,int> >& edgeEdgeCollisions()
  const   { return _edgeEdgeCollisions; }; const vector<bool>&
  edgeEdgeIntersections() const          { return _edgeEdgeIntersections; };
  const vector<pair<VECTOR2,VECTOR2> >& edgeEdgeCoordinates() const   { return
  _edgeEdgeCoordinates; }; const vector<REAL>& surfaceTriangleAreas() const {
  return _surfaceTriangleAreas; }; const vector<VECTOR3I>&
  surfaceTriangleNeighbors() const { return _surfaceTriangleNeighbors; };
  */
  ...
  */
  ...
  computeStretchingClampedHessian(const SHELL::STRETCHING &hyperelastic) const;
  virtual SPARSE_MATRIX
  computeStretchingHessian(const SHELL::STRETCHING &hyperelastic) const;
  ...
  SPARSE_MATRIX
  computeBendingClampedHessian(const SHELL::BENDING &hyperelastic) const;

  /*
  // compute damping quantities
  VECTOR computeDampingForces(const VOLUME::DAMPING& damping) const;
  virtual SPARSE_MATRIX computeDampingHessian(const VOLUME::DAMPING& damping)
  const;

  // compute collision quantities
  //VECTOR computeCollisionForces(const REAL& collisionStiffness) const;
  //SPARSE_MATRIX computeCollisionClampedHessian(const REAL& collisionStiffness)
  const;
  */
  ...
  // VECTOR computeEberleDampingForces(const VECTOR& velocity, const REAL&
  // integratorConstant,
  //                                   const REAL& collisionStiffness) const;
  // SPARSE_MATRIX computeEberleClampedHessian(const VECTOR& velocity, const
  // REAL& integratorConstant,
  //                                           const REAL& collisionStiffness)
  //                                           const;

  REAL computeEdgeEdgeCollisionEnergy() const;
  VECTOR computeEdgeEdgeCollisionForces() const;
  // VECTOR computeEdgeEdgeDampingForces(const VECTOR& velocity, const REAL&
  // integratorConstant,
  //                                     const REAL& collisionStiffness) const;
  SPARSE_MATRIX computeEdgeEdgeCollisionClampedHessian() const;
  // SPARSE_MATRIX computeEdgeEdgeDampingClampedHessian(const VECTOR& velocity,
  ...
  // Takes all the ee and vf CCDs and gathers them into _allCCDs in
  // chronological order updates _allCCDs
  void gatherCCDs();
  ...
  */
  ...
  static bool writeSurfaceToObj(const string& filename, const TRIANGLE_MESH&
  tetMesh);
  */

  // read in OBJ as a triangle mesh
  static bool readObjFile(const string &filename, vector<VECTOR3> &vertices,
                          vector<VECTOR3I> &triangles);

  // write out OBJ mesh file, the bool tells whether to write the
  // rest or deformed vertices
  static bool writeObjFile(const string &filename, const TRIANGLE_MESH &triMesh,
                           const bool restVertices);

  // normalize vertices so that they're in a unit box, centered at (0.5, 0.5,
  // 0.5)
  ...
  // see if the projection of v onto the plane of v0,v1,v2 is inside the
  // triangle formed by v0,v1,v2
  ...
  REAL surfaceFaceDihedralAngle(const int surfaceID0, const int surfaceID1)
  const;
  */
...
protected:
  ...
  */
  ...
  static VECTOR3 pointPlaneProjection(const vector<VECTOR3> &plane,
                                      const VECTOR3 &point);

  // compute distance to collision cell wall, where positive means inside
  // and negative means outside
  // REAL distanceToCollisionCellWall(const int triangleID, const VECTOR3&
  // vertex);
  ...
  _surfaceTriangles[triangleID]; VECTOR3 surfaceTriangleNormal(const int
  triangleID) const;
  ...
  */
  ...
  /*
  // for each pair of _surfaceEdges, what _collisionEps should we use? If they
  started
  ...
  */
  ...
  // .second represents the index into _vertices with 5th int as type (0 for vf,
  // 1 for ee) sorted chronologically at population time in gatherCCDs()
  vector<pair<REAL, VECTOR5I>> _allCCDs;

  // Each element in overlying vector represents a contiguous region.
  // Individual vectors are collections of indices into _allCCDs and should be
  // sorted chrono
  vector<vector<int>> _regionsCCD;
  // Direct helper with _regionsCCD: each vector element is a zone
  // each individual vector contains the set of all indexes into _vertices that
  // make up that zone
  ...
  // are the edge-edge collisions still separate, or is there already a
  // face-edge intersection?
  vector<bool> _edgeEdgeIntersections;

  // list of collisionEps for each _edgeEdgeIntersections. Usually this will be
  // the default, but if it started out closer than that, we use that instead
