...
namespace HOBAK {
using namespace std;
TRIANGLE_MESH_FASTER::TRIANGLE_MESH_FASTER(const vector<VECTOR3> &restVertices,
...
void TRIANGLE_MESH_FASTER::computeEdgeEdgeCollisions() {
  ...
  for (unsigned int x = 0; x < _edges.size(); x++) {
    ...
    for (unsigned int y = 0; y < coarseEdges.size(); y++) {
      ...
      // (a,b) and (b,a)
      if ((unsigned int)coarseEdges[y] < x)
        continue;
...
void TRIANGLE_MESH_FASTER::computeVertexFaceCollisions() {
  ...
  for (unsigned int x = 0; x < _vertices.size(); x++) {
    ...
    _aabbTreeTriangles.nearbyTriangles(surfaceVertex, _collisionEps,
                                       broadPhaseFaces);
