...
// into ANGLE so far
#ifndef FIELDVIEW3D
#define FIELDVIEW3D(x)                                                         \
  FIELD_3D::fieldViewer(x, VARNAME(x));                                        \
  sleep(1);
#endif
#ifndef OVERLAYFIELDVIEW3D
#define OVERLAYFIELDVIEW3D(x, y)                                               \
  FIELD_3D::overlayFieldViewer(x, y, VARNAME(x));                              \
  sleep(1);
#endif
#ifndef FIELDVIEW3DYZ
#define FIELDVIEW3DYZ(x)                                                       \
  FIELD_3D::fieldViewerYZ(x, VARNAME(x));                                      \
  sleep(1);
#endif
#ifndef OVERLAYFIELDVIEW3DYZ
#define OVERLAYFIELDVIEW3DYZ(x, y)                                             \
  FIELD_3D::overlayFieldViewerYZ(x, y, VARNAME(x));                            \
  sleep(1);
#endif
...
public:
  FIELD_3D();
  FIELD_3D(const int &xRes, const int &yRes, const int &zRes,
           const VECTOR3 &center = VECTOR3(0, 0, 0),
           const VECTOR3 &lengths = VECTOR3(1, 1, 1));
  FIELD_3D(const double *data, const int &xRes, const int &yRes,
  ...
  const int maxRes() const {
    return (_xRes > _yRes) ? ((_zRes > _xRes) ? _zRes : _xRes)
                           : ((_zRes > _yRes) ? _zRes : _yRes);
  };
  ...
  static void readHoudiniVel(const std::string filename, FIELD_3D &xVelocity,
  ...
  // do a union with 'field', assuming both this and field are signed distance
  // fields
  ...
  // assuming that SURFACE.initializeSignedDistanceField() has been called on
  // this field, do the fast marching method
  void fastMarchingMethod();

  // only fast march the inside (negative SDF) of the fluid to handle immersed
  // obstacles
  void fastMarchingNegativeOnly();

  // only fast march the outside (positive SDF) of the fluid to handle immersed
  // obstacles
  void fastMarchingPositiveOnly();

  // extend some scalar quantity off of a front, given a signed distance
  // function
  void fastExtension(const FIELD_3D &signedDistance);

  // return the indices of the grid points insides a world-space bounding box
  void boundingBoxIndices(const VECTOR3 &mins, const VECTOR3 &maxs,
  ...
  static void overlayFieldViewer(const FIELD_3D &field,
                                 const FIELD_3D &distance, std::string name);
  static void overlayFieldViewerYZ(const FIELD_3D &field,
                                   const FIELD_3D &distance, std::string name);
  ...
  void setToSolidCheckboard(int xChecks = 10, int yChecks = 10,
                            int zChecks = 10);
  void setToGrayCheckerboard(int xChecks = 10, int yChecks = 10,
                             int zChecks = 10);

  // set to vertical derivative kernel
  void setToVerticalDerivativeKernel(double kMax = 10, double dk = 0.01,
                                     double sigma = 1.0, double L = 1);

  // set whole field to a Gaussian
  void setToGaussian(REAL amplitude = 1.0,
                     VECTOR3 sigmas = VECTOR3(0.1, 0.1, 0.1));

  // set to a Gaussian
  void insertGaussian(const VECTOR3 &center, const REAL amplitude = 1.0,
                      const VECTOR3 sigmas = VECTOR3(0.1, 0.1, 0.1));
  ...
  FIELD_3D convolveNarrowBand(const FIELD_3D &filter, const FIELD_3D &distance,
                              int maxCells);
  FIELD_3D convolveNarrowBand(const FIELD_3D &filter,
                              const std::vector<int> &narrowBand);
  FIELD_3D convolveNarrowBandFast15(const FIELD_3D &filter,
                                    const std::vector<int> &narrowBand);
  FIELD_3D convolveNarrowBandFast15(const FIELD_3D &filter,
                                    const FIELD_3D &distance,
                                    const REAL maxRadius);

  // stomp all the value outside a narrow band to zero in order to boost
  // compression
  void stompOutsideNarrowBand(const std::vector<int> &narrowBand);

  // compute the narrow band indices for this object, which is assumed to be a
  // SDF the returned vector is (x,y,z) triplets
  ...
  // on the border, difference falls back to first order (not centered)
  // difference
  ...
  void principalCurvatures(FIELD_3D &minCurvature,
                           FIELD_3D &maxCurvature) const;

  // compute laplace everywhere (default factor scales to similar wave prop.
  // speed like iwave for wave equation)
  ...
  FIELD_3D resampleCubicUnclampedNarrowBand(int upResFactor,
                                            const FIELD_3D &distanceField,
                                            const int maxRadius) const;
  ...
  FIELD_3D narrowBandDiff(const FIELD_3D &rhs,
                          const std::vector<int> &narrowBand) const;

  // do a diff between two fields, but only along a narrow band, and not along
  // borders where the filter skipped
  FIELD_3D filteredNarrowBandDiff(const FIELD_3D &rhs,
                                  const std::vector<int> &narrowBand,
  ...
  void setInterval(const int xMin, const int xMax, const int yMin,
