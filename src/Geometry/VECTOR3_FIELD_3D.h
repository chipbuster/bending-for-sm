...
#define VECTORFIELDVIEW3D(x, y)                                                \
  VECTOR3_FIELD_3D::fieldViewer(x, y, VARNAME(x));                             \
  sleep(1);
#endif

#ifndef OVERLAYVECTORFIELDVIEW3D
#define OVERLAYVECTORFIELDVIEW3D(x, y)                                         \
  VECTOR3_FIELD_3D::overlayFieldViewer(x, y, VARNAME(x));                      \
  sleep(1);
#endif

#ifndef CLOSESTPOINTVIEW3D
#define CLOSESTPOINTVIEW3D(x, y)                                               \
  VECTOR3_FIELD_3D::closestPointViewer(x, y, VARNAME(x));                      \
  sleep(1);
#endif
...
public:
  VECTOR3_FIELD_3D();
  VECTOR3_FIELD_3D(const int &xRes, const int &yRes, const int &zRes,
                   const VECTOR3 &center = VECTOR3(0, 0, 0),
                   const VECTOR3 &lengths = VECTOR3(1, 1, 1));
  VECTOR3_FIELD_3D(double *data, const int &xRes, const int &yRes,
                   const int &zRes, const VECTOR3 &center = VECTOR3(0, 0, 0),
                   const VECTOR3 &lengths = VECTOR3(1, 1, 1));
  ...
  // reset the lengths to something else, and recompute all the dimesions as
  // well
  ...
  static FIELD_3D compose(const FIELD_3D &values,
                          const VECTOR3_FIELD_3D &positions);
  static VECTOR3_FIELD_3D compose(const VECTOR3_FIELD_3D &values,
                                  const VECTOR3_FIELD_3D &positions);
  ...
  // extend some vector quantity off of a front, given a signed distance
  // function
  void fastExtension(const FIELD_3D &signedDistance);

  // set the values in the field to the values at the closest points
  static FIELD_3D
  setToClosestPointValues(const FIELD_3D &input,
                          const VECTOR3_FIELD_3D &closestPoints);
  static FIELD_3D
  setToClosestPointValuesNarrowBand(const FIELD_3D &input,
                                    const VECTOR3_FIELD_3D &closestPoints,
                                    const FIELD_3D &distance, int maxCells);
  static FIELD_3D setToClosestPointValuesNarrowBandFrozenCore(
      const FIELD_3D &input, const VECTOR3_FIELD_3D &closestPoints,
      const FIELD_3D &distance, int maxRadius, int coreRadius = 1);
  static FIELD_3D setToClosestPointValuesFrozenCore(
      const FIELD_3D &input, const VECTOR3_FIELD_3D &closestPoints,
      const FIELD_3D &distance, int coreRadius = 1);
  ...
  void drawZSlice(const VECTOR3_FIELD_3D& origins, const int zSlice, const REAL
  scale = 1, const int stride = 4) const; void drawClosestPointZSlice(const
  VECTOR3_FIELD_3D& origins, const int zSlice, const REAL scale = 1, const int
  stride = 4) const;
  */

  // dump to a viewer
  static void fieldViewer(const VECTOR3_FIELD_3D &field,
                          const FIELD_3D &distanceField, string name);
  static void overlayFieldViewer(const VECTOR3_FIELD_3D &field,
                                 const FIELD_3D &distanceField, string name);
  static void closestPointViewer(const VECTOR3_FIELD_3D &field,
                                 const FIELD_3D &distanceField, string name);
  ...
  static void advect(const REAL dt, const VECTOR3_FIELD_3D &velocityGrid,
                     const FIELD_3D &oldField, FIELD_3D &newField);
  static void advectNarrowBand(const REAL dt,
                               const VECTOR3_FIELD_3D &velocityGrid,
                               const FIELD_3D &oldField, FIELD_3D &newField,
                               const FIELD_3D &distance, const int maxCells);
  static void
  advectNarrowBandLinear(const REAL dt, const VECTOR3_FIELD_3D &velocityGrid,
                         const FIELD_3D &oldField, FIELD_3D &newField,
                         const FIELD_3D &distance, const int maxCells);
  static void advectLazy(const REAL dt, const VECTOR3_FIELD_3D &velocityGrid,
                         const FIELD_3D &distanceOld,
                         const FIELD_3D &distanceNew, const int maxCells,
                         const FIELD_3D &oldField, FIELD_3D &newField);

  // advect with second order MacCormack
  static void advectMacCormack(const REAL dt,
                               const VECTOR3_FIELD_3D &velocityGrid,
                               FIELD_3D &oldField, FIELD_3D &newField,
                               FIELD_3D &temp1, FIELD_3D &temp2);
  static void
  advectMacCormackLazy(const REAL dt, const VECTOR3_FIELD_3D &velocityGrid,
  ...
  static void advectMacCormackNarrowBand(
  ...
  // copy values out into the border, assuming that "borderSize" is the width of
  // the grid padding
  ...
  static VECTOR3_FIELD_3D computeClosestPoints(const FIELD_3D &input,
                                               const FIELD_3D &surfaceField,
                                               bool verbose = true);
  static void computeClosestPointsNarrowBand(const FIELD_3D &input,
                                             const FIELD_3D &surfaceField,
                                             const int maxCells,
                                             VECTOR3_FIELD_3D &final);

  // compute the extension field for some subset of points
  static FIELD_3D computeExtensionFieldSubset(const FIELD_3D &distanceField,
                                              const FIELD_3D &toExtend,
                                              const vector<int> &subset);

  // compute the extension field for some subset of points
  static FIELD_3D computeExtensionFieldMasked(const FIELD_3D &distanceField,
                                              const FIELD_3D &toExtend,
                                              const FIELD_3D &mask);
  static FIELD_3D
  computeExtensionFieldMaskedBadAdvect(const FIELD_3D &distanceField,
                                       const FIELD_3D &toExtend,
                                       const FIELD_3D &mask);

  // do an advection of a subset of the grid
  static FIELD_3D advectSubset(const REAL dt,
                               const VECTOR3_FIELD_3D &velocityGrid,
                               const FIELD_3D &oldField,
                               const vector<int> &subset);

  // set the closes point values needed for MacCormack
  static FIELD_3D setMacCormackClosestPoints(
      const REAL dt, const VECTOR3_FIELD_3D &velocityGrid,
      const FIELD_3D &distanceOld, const FIELD_3D &distanceNew,
      const int maxCells, const FIELD_3D &oldField);

  // Clamp the extrema generated by the BFECC error correction
  static void clampExtrema(const REAL dt, const VECTOR3_FIELD_3D &velocityField,
                           const FIELD_3D &oldField, FIELD_3D &newField);
  static void clampOutsideRays(const REAL dt,
                               const VECTOR3_FIELD_3D &velocityField,
                               const FIELD_3D &oldField,
                               const FIELD_3D &oldAdvection,
                               FIELD_3D &newField);
  static void clampExtremaSubset(const REAL dt,
                                 const VECTOR3_FIELD_3D &velocityField,
                                 const FIELD_3D &oldField, FIELD_3D &newField,
                                 const vector<int> &subset);
  static void
  clampOutsideRaysSubset(const REAL dt, const VECTOR3_FIELD_3D &velocityField,
                         const FIELD_3D &oldField, const FIELD_3D &oldAdvection,
                         FIELD_3D &newField, const vector<int> &subset);
...
private:
  ...
  static REAL getClosestPointValue(const VECTOR3_FIELD_3D &targetGradient,
