...
public:
  ...
  STRAND_MESH(const vector<VECTOR3> &restVertices,
              ...
              const REAL &density, const REAL &radiusA, const REAL &radiusB);
  ...
  const vector<pair<int, int>> &edgeEdgeCollisions() const {
    return _edgeEdgeCollisions;
  };
  const vector<pair<VECTOR2, VECTOR2>> &edgeEdgeCoordinates() const {
    return _edgeEdgeCoordinates;
  };
  const vector<pair<int, int>> &edgeEdgeCollisionsOld() const {
    return _edgeEdgeCollisionsOld;
  };
  const vector<pair<VECTOR2, VECTOR2>> &edgeEdgeCoordinatesOld() const {
    return _edgeEdgeCoordinatesOld;
  };
  ...
  virtual const int DOFs() const {
    return _vertices.size() * 3 + _edges.size();
  };
  ...
  virtual REAL
  computeStretchingEnergy(const STRAND::STRETCHING &hyperelastic) const;
  virtual VECTOR
  computeStretchingForces(const STRAND::STRETCHING &hyperelastic) const;
  virtual SPARSE_MATRIX
  computeStretchingHessian(const STRAND::STRETCHING &hyperelastic) const;
  virtual SPARSE_MATRIX
  computeStretchingClampedHessian(const STRAND::STRETCHING &hyperelastic) const;
...
protected:
  ...
  SPARSE_MATRIX
  buildPerBendMatrix(const vector<MATRIX11> &perBendHessians) const;
  SPARSE_MATRIX
  buildPerEdgeMatrix(const vector<MATRIX6> &perEdgeHessians) const;
  virtual SPARSE_MATRIX
  buildEdgeEdgeMatrix(const vector<MATRIX12> &perEdgeHessians) const;
  ...
  SPARSE_MATRIX
  buildPerBendMatrixEdgeEnd(const vector<MATRIX11> &perBendHessians) const;
  SPARSE_MATRIX
  buildPerEdgeMatrixEdgeEnd(const vector<MATRIX6> &perEdgeHessians) const;
  SPARSE_MATRIX
  buildEdgeEdgeMatrixEdgeEnd(const vector<MATRIX12> &perEdgeHessians) const;
  ...
  VECTOR
  buildEdgeEdgeVectorEdgeEnd(const vector<VECTOR12> &perEdgeForces) const;

  // version that interleaves the edge indices
  SPARSE_MATRIX
  buildPerBendMatrixInterleaved(const vector<MATRIX11> &perBendHessians) const;
  SPARSE_MATRIX
  buildPerEdgeMatrixInterleaved(const vector<MATRIX6> &perEdgeHessians) const;
  SPARSE_MATRIX
  buildEdgeEdgeMatrixInterleaved(const vector<MATRIX12> &perEdgeHessians) const;
  VECTOR
  buildPerBendVectorInterleaved(const vector<VECTOR11> &perBendForces) const;
  VECTOR
  buildPerEdgeVectorInterleaved(const vector<VECTOR6> &perEdgeForces) const;
  VECTOR
  buildEdgeEdgeVectorInterleaved(const vector<VECTOR12> &perEdgeForces) const;
  ...
  VECTOR _restTwistAngles;  // _totalEdges, though seems like it should be
                            // _totalVertices
