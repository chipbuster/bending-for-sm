...
// are trying to hit. Assumes that buildConstraintMatrix() has already been
// called
///////////////////////////////////////////////////////////////////////////////////////////////////////
void TIMESTEPPER::updateConstraintTargets() {
  // std::cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " <<
  // std::endl; cout << " Constraint targets: " << _planeConstraints.size() <<
  // endl;
...
//
// NOTE: this is different from the quasistatic case, in that it stores the
// kinematic corrections in _position, and not directly the vertices of
// _tetMesh. When the displacement of _tetMesh is pinned during the Newton
// solve, then the _teMesh will see the constraints.
//
...
// but using the "Smoothed aggregation multigrid for cloth simulation"
// projection matrix
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool TIMESTEPPER::solveQuasistatic(const bool verbose) {
  ...
  const VECTOR stretchingForce =
      _strandMesh.computeStretchingForces(_stretchingEnergy);
  ...
  // std::cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " <<
  // std::endl; std::cout << " Internal forces: " << std::endl; std::cout <<
  // "mine = " << R.transpose().format(octave) << std::endl; std::cout <<
  // "stretchingForceMine = " << stretchingForce.transpose().format(octave) <<
  // std::endl; std::cout << "bendingForceMine = " <<
  // bendingForce.transpose().format(octave) << std::endl; std::cout <<
  // "twistingForceMine = " << twistingForce.transpose().format(octave) <<
  // std::endl;
  ...
  // SPARSE_MATRIX K =
  // _strandMesh.computeStretchingClampedHessian(_stretchingEnergy); K +=
  // _strandMesh.computeBendingClampedHessian(); K +=
  // _strandMesh.computeTwistingClampedHessian(); cout << " stretchingMine =  "
  // <<
  // MATRIX(_strandMesh.computeStretchingClampedHessian(_stretchingEnergy)).format(octave)
  // << endl; cout << " bendingMine =  " <<
  // MATRIX(_strandMesh.computeBendingClampedHessian()).format(octave) << endl;
  // cout << " twistingMine =  " <<
  // MATRIX(_strandMesh.computeTwistingClampedHessian()).format(octave) << endl;
...
VECTOR TIMESTEPPER::solveSystem(const SPARSE_MATRIX &LHS, const VECTOR &rhs) {
...
#ifdef EIGEN_USE_MKL_ALL
  {
    ...
    Eigen::PardisoLLT<SPARSE_MATRIX> solver;  // this one is mildly faster than
LDLT
...
bool TIMESTEPPER::solveDynamics(const bool verbose) {
  ...
  if (_hessianClampingEnabled) {
    ...
    // SPARSE_MATRIX K =
    // _strandMesh.computeStretchingHessian(_stretchingEnergy); K +=
    // _strandMesh.computeBendingHessian(); K +=
    // _strandMesh.computeTwistingHessian();
...
bool TIMESTEPPER::solveNewton(const bool verbose) {
  ...
  while ((step < _maxNewtonIterations && maxR > eps) ||
...
#if 1
    SPARSE_MATRIX K =
        _strandMesh.computeStretchingClampedHessian(_stretchingEnergy);
    K += _strandMesh.computeBendingClampedHessian();
    K += _strandMesh.computeTwistingClampedHessian();
#else
    SPARSE_MATRIX K = _strandMesh.computeStretchingHessian(_stretchingEnergy);
    ...
    VECTOR acceleration =
        -alpha0 * (_position - _positionOld) + alpha1 * _velocityOld;
    ...
    if (verbose)
      printf("  PCG iters: %3i err: %6.4e \n", (int)_cgSolver.iterations(),
             (float)_cgSolver.error());
#else
    TIMER choleskyTimer("Cholesky Solve");
    Eigen::SimplicialLDLT<SPARSE_MATRIX> solver;
    solver.compute(LHS);
    ...
    // see if any of the constraints changed. Used to be that this was outside
    // the Newton loop because the behavior was too oscillatory, but causes too
    // many penetrations to slip through when the Poisson's ratio gets high
    if (constraintsChanged) {
      deleteSurfaceConstraints(verbose);
      updateSurfaceConstraints();
      buildConstraintMatrix();
...
bool TIMESTEPPER::findSeparatingSurfaceConstraints(const VECTOR &unfiltered) {
  ...
  for (unsigned int x = 0; x < _planeConstraints.size(); x++) {
    ...
    const int vertexID = constraint.vertexID;
    const REAL signedDistance =
        shape->signedDistance(_strandMesh.vertices()[vertexID]);
    ...
    */
    ...
    // make the test material agnostic and only look at the direction; then if
    // it's a big force, the testing threshold won't get messed up later
    ...
    if (debug)
      cout << "  separation magnitude: " << separationMagnitude << endl;

    // if the distance is outside and large, move on
    // if (signedDistance > 1e-4 || (signedDistance > 1e-6 &&
    // separationMagnitude > 1e-6))
    ...
    // if (signedDistance > 1e-4)
    {
      ...
      changed = true;
      if (debug)
        cout << " CONSTRAINT IS OUTSIDE " << endl;
      continue;
    }
    ...
    */
    ...
    if (separationMagnitude >
        1e-6) // velocity condition doesn't seem to ever get triggered
    ...
    // if (separationMagnitude > 1e-4 || velocitySeparation > 1e-4)  // jitter
    // goes away for SNH lambda = 1000, but gets sticky if (separationMagnitude
    // > 9e-5 || velocitySeparation > 9e-5)
    {
      // cout << " Separation: " << separationMagnitude << " velocity: " <<
      // velocitySeparation << endl;
      constraint.isSeparating = true;
      changed = true;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
void TIMESTEPPER::attachKinematicSurfaceConstraints(
    const KINEMATIC_SHAPE *shape, const bool verbose) {
  ...
  if (verbose)
    cout << " Constrained " << _kinematicConstraints.size() << " vertices "
         << endl;
...
void TIMESTEPPER::findNewSurfaceConstraints(const bool verbose) {
  ...
  if (verbose)
    cout << " Currently tracking " << _planeConstraints.size()
         << " plane constraints " << endl;
  ...
  for (unsigned int y = 0; y < _collisionObjects.size(); y++) {
    ...
    for (unsigned int x = 0; x < vertices.size(); x++) {
      ...
      if (_inCollision[vertexID]) {
        if (debug)
          cout << " vertex is already collision, moving on" << endl;
        continue;
      }
      ...
      if (!shape->inside(vertex)) {
        if (debug)
          cout << " vertex is not inside the shape, moving on" << endl;
        continue;
      }
      ...
      // sliding
      //
      // BDF-2 sticks unless -FLT_EPSILON is used, but other integrators seem
      // okay
...
void TIMESTEPPER::deleteSurfaceConstraints(const bool verbose) {
  ...
  //
  // That's right, I'm just building a whole new vector instead of deleting
  // nodes from a linked list. If it's all too ugly for you, look away. Like I
  // said in the README.md, this library is not optimized yet.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
void TIMESTEPPER::computeCollisionResponse(VECTOR &R, SPARSE_MATRIX &K,
                                           SPARSE_MATRIX &collisionC,
                                           const bool verbose) {
  TIMER functionTimer(__FUNCTION__);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void TIMESTEPPER::applyRigidRotation(const MATRIX3 &R,
