...
TIMESTEPPER::TIMESTEPPER(TRIANGLE_MESH &triangleMesh,
                         SHELL::STRETCHING &stretching, SHELL::BENDING &bending)
    : _triangleMesh(triangleMesh), _stretchingEnergy(stretching),
      _bendingEnergy(bending) {
  initialize();
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// [TJM15] refers to "Smoothed Aggregation Multigrid for Cloth Simulation",
// SIGGRAPH Asia 2015
//
...
bool TIMESTEPPER::solve(const bool verbose) {
  ...
  SPARSE_MATRIX K =
      _triangleMesh.computeStretchingClampedHessian(_stretchingEnergy);
  SPARSE_MATRIX bendingK =
      _triangleMesh.computeBendingClampedHessian(_bendingEnergy);
  K += bendingK;
...
bool TIMESTEPPER::findSeparatingSurfaceConstraints(const VECTOR &unfiltered) {
  ...
  for (unsigned int x = 0; x < _planeConstraints.size(); x++) {
    ...
    const REAL signedDistance =
        shape->signedDistance(_triangleMesh.vertices()[vertexID]);
