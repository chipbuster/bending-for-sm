...
TAN_BENDING::TAN_BENDING(const REAL &mu) : ISOTROPIC_BENDING(mu) {}

///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
std::string TAN_BENDING::name() const { return std::string("TAN_BENDING"); }
...
REAL TAN_BENDING::psi(const MATRIX3x2 &E, const REAL &theta0,
...
MATRIX3x2 TAN_BENDING::PK1(const MATRIX3x2 &E, const REAL &theta0,
                           const bool inverted) const {
  // theta
  const VECTOR3 e0 = E.col(0);
  const VECTOR3 e1 = E.col(1);
...
MATRIX6 TAN_BENDING::hessian(const MATRIX3x2 &E, const REAL &theta0) const {
  ...
  MATRIX3 B00 =
      (dot0 * N - 2.0 * e0perp * e0.transpose() + (-E0 * pnpe0) * dot0) / dot0 /
      dot0;
  MATRIX3 B01 = -E0 * pnpe1 / dot0;
  MATRIX3 B11 =
      (-dot1 * N + 2.0 * e1perp * e1.transpose() + (E1 * pnpe1) * dot1) / dot1 /
      dot1;
  MATRIX3 B10 = E1 * pnpe0 / dot1;
  MATRIX6 H;
  H << B00, B01, B10, B11;

  const REAL cosine = cos(theta * 0.5), sine = sin(theta * 0.5),
             tangent = tan(theta * 0.5);
  // partial psi partial theta
  REAL g, h;
  if (abs(cosine) < 1e-16) {
    g = 0.0;
    ...
    h = _mu * (0.5 + sine * sine - tan(theta0 * 0.5) * cosine * sine) / cosine /
        cosine / cosine / cosine;
  }
  return h * G * G.transpose() + g * H;
}

MATRIX6 TAN_BENDING::hessian(const MATRIX3x2 &E, const REAL &theta0,
                             const bool inverted) const {
  const VECTOR3 e0 = E.col(0);
  const VECTOR3 e1 = E.col(1);
  const REAL cosTheta = clamp(e0.dot(e1) / (e0.norm() * e1.norm()), -1.0, 1.0);
...
MATRIX6 TAN_BENDING::clampedHessian(const MATRIX3x2 &E,
                                    const REAL &theta0) const {
  const VECTOR3 e0 = E.col(0);
  const VECTOR3 e1 = E.col(1);
  const REAL cosTheta = clamp(e0.dot(e1) / (e0.norm() * e1.norm()), -1.0, 1.0);
  ;
  ...
  if (abs(theta) < 1e-16) {
    ...
    lambda4 =
        g * (1.0 / dot1 / tan(theta) - alpha / norm0 / norm1 / sin(theta));
    lambda5 = g * (1.0 / dot1 / tan(theta) +
                   1.0 / alpha / norm0 / norm1 / sin(theta));
  }
  // the rest 4
  ...
  MATRIX6x4 A;
  A << e0, e0perp, VECTOR3::Zero(), VECTOR3::Zero(), VECTOR3::Zero(),
      VECTOR3::Zero(), e1, e1perp;
  ...
  VECTOR6 lambda;
  lambda << lambda0, lambda1, lambda2, lambda3, lambda4, lambda5;
...
MATRIX6 TAN_BENDING::clampedHessian(const MATRIX3x2 &E, const REAL &theta0,
                                    const bool inverted) const {
  const VECTOR3 e0 = E.col(0);
  const VECTOR3 e1 = E.col(1);
  const REAL cosTheta = clamp(e0.dot(e1) / (e0.norm() * e1.norm()), -1.0, 1.0);
  ...
  if (abs(theta) < 1e-16) {
    ...
    lambda4 = sign * g *
              (1.0 / dot1 / tan(theta) - alpha / norm0 / norm1 / sin(theta));
    lambda5 =
        sign * g *
        (1.0 / dot1 / tan(theta) + 1.0 / alpha / norm0 / norm1 / sin(theta));
  }
  // the rest 4
