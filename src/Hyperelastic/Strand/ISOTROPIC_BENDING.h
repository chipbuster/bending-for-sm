...
public:
  ...
  ISOTROPIC_BENDING(const REAL &mu, const REAL &theta0) {
    _mu = mu;
    _theta0 = theta0;
  };
  ...
  virtual MATRIX6 clampedHessian(const MATRIX3x2 &E,
                                 const REAL &theta0) const = 0;
  virtual REAL psi(const MATRIX3x2 &E, const REAL &theta0,
                   const bool inverted) const = 0;
  virtual MATRIX3x2 PK1(const MATRIX3x2 &E, const REAL &theta0,
                        const bool inverted) const = 0;
  virtual MATRIX6 hessian(const MATRIX3x2 &E, const REAL &theta0,
                          const bool inverted) const = 0;
  virtual MATRIX6 clampedHessian(const MATRIX3x2 &E, const REAL &theta0,
                                 const bool inverted) const = 0;
  ...
  MATRIX6 clampedHessian(const MATRIX3x2 &E) const {
    return clampedHessian(E, _theta0);
  };

  REAL psi(const MATRIX3x2 &E, const bool inverted) const {
    return psi(E, _theta0, inverted);
  };
  MATRIX3x2 PK1(const MATRIX3x2 &E, const bool inverted) const {
    return PK1(E, _theta0, inverted);
  };
  MATRIX6 hessian(const MATRIX3x2 &E, const bool inverted) const {
    return hessian(E, _theta0, inverted);
  };
  MATRIX6 clampedHessian(const MATRIX3x2 &E, const bool inverted) const {
    return clampedHessian(E, _theta0, inverted);
  };
