...
///////////////////////////////////////////////////////////////////////
VECTOR12
VERTEX_FACE_COLLISION::gradient(const vector<VECTOR3> &vertices) const {
...
MATRIX12 VERTEX_FACE_COLLISION::springLengthHessian(const vector<VECTOR3> &v,
  ...
  const MATRIX12 contracted =
      tvf[0] * normalH[0] + tvf[1] * normalH[1] + tvf[2] * normalH[2];
...
MATRIX12 VERTEX_FACE_COLLISION::hessian(const VECTOR12 &x) const {
  ...
  return 2.0 * _mu * (gvf * gvf.transpose() + springLength * springLengthH);
