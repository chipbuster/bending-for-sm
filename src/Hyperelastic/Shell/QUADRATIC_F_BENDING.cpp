...
QUADRATIC_F_BENDING::QUADRATIC_F_BENDING(const REAL &mu) : BENDING(mu) {}
...
REAL QUADRATIC_F_BENDING::psi(const vector<VECTOR3> &flap,
  ...
  const REAL theta =
      s *
      (M_PI - acos(clampValue(z0.dot(z1) / z0.norm() / z1.norm(), 1.0, -1.0)));
  const REAL diff = theta - restTheta;

  return 0.5 * _mu * diff * diff;
}

VECTOR12 QUADRATIC_F_BENDING::gradient(const std::vector<VECTOR3> &flap,
                                       const REAL &restTheta) const {
  // compute theta
  const VECTOR3 y1 = flap[2] - flap[1];
  const VECTOR3 y2 = flap[0] - flap[1];
  ...
  const REAL theta =
      s * (M_PI - acos(clampValue(z0.dot(z1) / normz0 / normz1, 1.0, -1.0)));
...
MATRIX12 QUADRATIC_F_BENDING::hessian(const std::vector<VECTOR3> &flap,
                                      const REAL &restTheta) const {
  // compute theta
  const VECTOR3 y1 = flap[2] - flap[1];
  const VECTOR3 y2 = flap[0] - flap[1];
  ...
  // const MATRIX3 eta2Diff = -(t1.dot(y2)*sigmaDiff + t1 *
  // z0.transpose())/norm1; const MATRIX3 eta3Diff = -(t1.dot(y3)*sigmaDiff + t1
  // * z1.transpose())/norm1; MATRIX6x12 pFpXDiff; pFpXDiff << sigmaDiff,
  // -eta2Diff-sigmaDiff, eta2Diff, z33,
  ...
  MATRIX6x12 pFpX;
  pFpX << sigma, -eta2 - sigma, eta2, z33, z33, -eta3 - sigma, eta3, sigma;
  ...
  G << g0, g1;
  // theta hessian
  const MATRIX3 Tb = getCrossMatrix(tb), T0 = getCrossMatrix(tau0),
                T1 = getCrossMatrix(tau1);
  MATRIX6 H;
  if (crs == 0) {
    H.setZero();
  } else {
    MATRIX3 B00, B01, B11, B10;
    B00 = 1.0 / normz0 / normz0 *
          (Tb - 2.0 * tau0perp * tau0.transpose() -
           (T0 * T1 + tau0perp * tau1perp.transpose()) / crs);
    B01 = 1.0 / crs / normz0 / normz1 *
          (T0 * T0 + tau0perp * tau0perp.transpose());
    B11 = 1.0 / normz1 / normz1 *
          (-Tb + 2.0 * tau1perp * tau1.transpose() -
           (T1 * T0 + tau1perp * tau0perp.transpose()) / crs);
    B10 = 1.0 / crs / normz0 / normz1 *
          (T1 * T1 + tau1perp * tau1perp.transpose());
    H << B00, B01, B10, B11;
  }
  ...
  const MATRIX3 t1tau0perp = t1 * tau0perp.transpose(),
                t1tau1perp = t1 * tau1perp.transpose();
  MATRIX3x12 tau0pSigmapx, tau1pSigmapx, eta2px0, eta3px0, tau0peta2px,
      tau1peta3px;
  VECTOR12 eta2px1, eta3px1;
  tau0pSigmapx << z33, t1tau0perp, -t1tau0perp, z33;
  tau1pSigmapx << z33, t1tau1perp, -t1tau1perp, z33;
  tau0pSigmapx *= 1.0 / norm1;
  tau1pSigmapx *= 1.0 / norm1;
  const VECTOR3 reflect2 = y2 - 2.0 * t1 * (t1.dot(y2)),
                reflect3 = y3 - 2.0 * t1 * (t1.dot(y3));
  eta2px0 << z33, reflect2 * tau0perp.transpose(),
      -reflect2 * tau0perp.transpose(), z33;
  eta2px1 << y1, -reflect2 - y1, reflect2, z3;
  eta3px0 << z33, reflect3 * tau1perp.transpose(),
      -reflect3 * tau1perp.transpose(), z33;
  eta3px1 << z3, -reflect3 - y1, reflect3, y1;
  tau0peta2px =
      1.0 / norm1 / norm1 * (eta2px0 - tau0perp * eta2px1.transpose());
  tau1peta3px =
      1.0 / norm1 / norm1 * (eta3px0 - tau1perp * eta3px1.transpose());
  // this computes p Psi p theta. should be computed inside the material class.
  // const REAL s = tb.dot(t1) > 0?-1:1;
  // const REAL thetaGrad = 2.0 * s * material->mu() * (acos(tau0.dot(tau1)) -
  // material->theta0());
  MATRIX12 extra;
  extra << 1.0 / normz0 * tau0pSigmapx,
      1.0 / normz0 * (-tau0peta2px - tau0pSigmapx) +
          1.0 / normz1 * (tau1peta3px + tau1pSigmapx),
  ...
  // MATRIX12 rank2 = pFpXSimp.transpose() * pPsipF * pFpXSimp -
  // pFpXDiff.transpose() * pPsipF * pFpXDiff; cout<<"extra: \n"<<(- s * g
  // *extra)<<endl; cout<<"rank2 extra: \n"<< (rank2 - pFpX.transpose() * pPsipF
  // * pFpX)<<endl;
  return rank4;
}

MATRIX12 QUADRATIC_F_BENDING::clampedHessian(const std::vector<VECTOR3> &flap,
                                             const REAL &restTheta) const {
...
//         z33, -eta3Simp-sigmaSimp, eta3Simp, sigmaSimp;
  ...
  // return pFpXSimp.transpose() * pPsipF * pFpXSimp - pFpXDiff.transpose() *
  // pPsipF * pFpXDiff;
  ...
  VECTOR6 lambda = VECTOR6::Zero(), lambdaPos = VECTOR6::Zero(),
          lambdaNeg = VECTOR6::Zero();
  ...
  if (abs(theta) < 1e-16) {
    ...
    lambda(4) =
        -g / sin(theta) * (-cos(theta) / dot1 - alpha / normz0 / normz1);
    lambda(5) =
        -g / sin(theta) * (-cos(theta) / dot1 + 1.0 / alpha / normz0 / normz1);
  }
  ...
  q0[1] =
      (nSum * (-n0 - 2.0 * w2 * nDiff) + n0 * r + 0.5 * (nDiff - r) * Rminus) /
      n1;
  q0[2] = 4.0 * n1 * w;
  q0[3] = nSum - r - Rminus;
  ...
  q2[1] =
      (nSum * (-n0 - 2.0 * w2 * nDiff) - n0 * r + 0.5 * (nDiff + r) * Rplus) /
      n1;
  q2[2] = 4.0 * n1 * w;
  q2[3] = nSum + r - Rplus;
  ...
  q3[3] = nSum + r + Rplus;
  MATRIX6x4 A;
  A << z0, z0perp, z3, z3, z3, z3, z1, z1perp;
  ...
  //         z33, -eta3Simp-sigmaSimp, eta3Simp, sigmaSimp;
  // pFpx original - pFpx simplified
  // return pFpXSimp.transpose() * filterPos * pFpXSimp - pFpXDiff.transpose() *
  // filterNeg * pFpXDiff;
  ...
  // const REAL d00 = (p0.dot(p0)) / (u0.dot(u0)), d01 = (p0.dot(p1)) /
  // (u1.dot(u1)); const REAL d10 = (p0.dot(p1)) / (u0.dot(u0)), d11 =
  // (p1.dot(p1)) / (u1.dot(u1));
  double x[4] = {0.0, 0.0, 0.0, 0.0};
  SolveP4De(x, -aa2 - bb2 * (d11 + d00), bb2 * aa * (d00 - d11),
            bb2 * bb2 * (d11 * d00 - d10 * d01));
  MATRIX12 clamped;
  clamped.setZero();
  // this computes p Psi p theta. should be computed inside the material class.
  ...
  for (int i = 0; i < 4; i++) {
    if (x[i] * thetaGrad > 0.0) {
      ...
      if (dotu0 < 1e-22 || abs(f) < 1e-22 || abs(d10) < 1e-22 ||
          abs(bb) < 1e-22)
        e = 0.0;
      else
...
#endif
  // MATRIX12 rank2 = pFpXSimp.transpose() * unfiltered * pFpXSimp -
  // pFpXDiff.transpose() * unfiltered * pFpXDiff; MATRIX12 extraTerm = rank2 -
  // pFpX.transpose() * unfiltered * pFpX;
  ...
  // cout<<"clamped eigenvalues: \n"<<thetaGrad*x[0]<<" "<<thetaGrad*x[1]<<"
  // "<<thetaGrad*x[2]<<" "<<thetaGrad*x[3]<<endl; cout<<"extraterm:
  // \n"<<extraTerm<<endl; cout<<"clamped: \n"<<clamped<<endl; exit(0); return
  // rank4;
  const MATRIX3 sigmaDiff = -t1 * t1.transpose();
  const MATRIX3 eta2Diff =
      -(t1.dot(y2) * sigmaDiff + t1 * z0.transpose()) / norm1;
  const MATRIX3 eta3Diff =
      -(t1.dot(y3) * sigmaDiff + t1 * z1.transpose()) / norm1;
  MATRIX6x12 pFpXDiff;
  pFpXDiff << sigmaDiff, -eta2Diff - sigmaDiff, eta2Diff, z33, z33,
      -eta3Diff - sigmaDiff, eta3Diff, sigmaDiff;
  const MATRIX6x12 pFpXSimp = pFpX - pFpXDiff;
  const MATRIX6 filterNeg = Q * lambdaNeg.asDiagonal() * Q.transpose();
  return pFpXSimp.transpose() * filterPos * pFpXSimp -
         pFpXDiff.transpose() * filterNeg * pFpXDiff;
}

REAL QUADRATIC_F_BENDING::restAngle(
    const std::vector<VECTOR3> &restFlap) const {
